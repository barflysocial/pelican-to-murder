...
function normalizeRounds(pack) {
  ...
  const fallback = [
    { id: 'r1', title: 'Round 1: The Body', shortTitle: 'Body', startSec: 0, endSec: 12 * 60, objective: 'Review the scene.', dialogue: 'The case begins now.' },
    { id: 'r2', title: 'Round 2: The Timeline', shortTitle: 'Timeline', startSec: 12 * 60, endSec: 24 * 60, objective: 'Rebuild the timeline.', dialogue: 'Track movement.' },
    { id: 'r3', title: 'Round 3: The Cover-Up', shortTitle: 'Cover-Up', startSec: 24 * 60, endSec: ACCUSATION_OPEN_SEC, objective: 'Follow the cover-up.', dialogue: 'The motive sharpens.' },
    { id: 'r4', title: 'Final Accusation', shortTitle: 'Accuse', startSec: ACCUSATION_OPEN_SEC, endSec: ACCUSATION_LOCK_SEC, objective: 'Submit your final accusation.', dialogue: 'Choose carefully.' }
  ];
...
function getBadge(score, total) {
  if (score >= total) return 'Lead Detective';
  if (score === 3) return 'Jr Detective';
  if (score === 2) return 'Rookie Detective';
  return "Don't Give Up Your Day Job";
}
...
function clientState(session) {
  ...
  return {
    ...
    accusation: pack.accusation || null,
    mode: 'Detective Mode',
...
function upsertSubmission(session, submission) {
  session.submissions = (session.submissions || []).filter(s => s.playerId !== submission.playerId);
...
app.post('/api/sessions/:code/accuse', async (req, res) => {
  ...
  for (const q of questions) {
    ...
    if (selected) answers[q.id] = selected;
  }
  const submission = {
    id: nanoid(8),
    playerId,
  ...
  upsertSubmission(session, submission);
  await saveSession(session);
...
async function startServer() {
  ...
  if (loaded === 0) {
    const defaultSession = createSession({ tableName: 'Pelican to Mars', truthPackId: 'pelican-to-murder' });
