...
function startGameFromBriefing(session, startedAt = Date.now()) {
  ...
  if (!alreadyHasStart) {
    addHostMessage(session, {
      ...
      text: 'The five-minute case setup is complete. The 30-minute investigation timer has begun. Open Forensics, Interrogation, Timeline, and Deduction as leads unlock.',
...
app.post('/api/sessions/:code/skip-briefing', async (req, res) => {
  ...
  session.revealed = false;
  session.results = [];
  const alreadyHasStart = (session.hostMessages || []).some(m => m.kind === 'briefing-skipped');
  if (!alreadyHasStart) {
    addHostMessage(session, {
      title: 'Investigation Started',
      text: 'The host skipped the case briefing. Begin the investigation now.',
      kind: 'briefing-skipped'
    });
  }
  await saveSession(session);
  res.json(clientState(session));
