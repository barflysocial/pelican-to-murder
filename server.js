...
function servePlayerWithRichPreview(req, res) {
  ...
  const imageUrl = `${origin}/assets/barfly-social-mystery-shared-title.png`;
...
function prepareDemoSession(session) {
  ...
  session.tableName = session.tableName || 'Easy — Fictional Baton Rouge Venue';
  ...
  if (!session.hostMessages.length) {
    ...
    addHostMessage(session, {
      ...
      text: pack.openingNarration || 'Barfly Mystery Template is live. Review the evidence as it unlocks and submit your accusation before time runs out.',
      kind: 'opening'
    });
  }
  return session;
...
function getOrCreateDemoSession() {
  ...
  if (!session) {
    session = createSession({
      tableName: 'Easy — Fictional Baton Rouge Venue',
  ...
  const items = Array.from(sessions.values())
    ...
    .map(session => {
      ...
      return {
        ...
        ticketPrice: normalizeTicketPrice(session.ticketPrice || session.eventPrice || ''),
        eventPriceLabel: eventPriceLabel(session),
        titleGraphic: pack.titleGraphic || pack.titleImage || pack.coverImage || '/assets/barfly-social-mystery-shared-title.png',
