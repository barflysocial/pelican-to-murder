...
function servePlayerWithRichPreview(req, res) {
  ...
  const imageUrl = `${origin}/assets/title-placeholder.png`;
...
function getPack(sessionOrId) {
  ...
  return truthPacks[id] || truthPacks['velvet-cypress-medium'] || truthPacks['velvet-cypress-easy'] || truthPacks.demo;
}
...
function clientState(session) {
  ...
  return {
    ...
    gameTitle: pack.gameTitle || 'Barfly Mystery Template',
    venue: pack.venue || 'Fictional Baton Rouge Venue',
...
function createSession({ tableName = 'Table 1', truthPackId = 'sample-episode-01-medium', playerCap = 25, eventDate = '', eventTime = '', eventDurationMinutes = 45, eventStatus = 'open', eventType = 'paid', ticketPrice = '', eventPrice = '', sharedAccessCode = '', allowLateCheckIn = false, scheduledStartAt = 0, autoStartEnabled = true } = {}) {
  let code = makeCode();
...
function prepareDemoSession(session) {
  ...
  session.tableName = session.tableName || 'Training Level 1 — Fictional Baton Rouge Venue';
  ...
  if (!session.hostMessages.length) {
    ...
    addHostMessage(session, {
      ...
      text: pack.openingNarration || 'Barfly Mystery Template Training Level 1 is live. Review the evidence as it unlocks and submit your accusation before time runs out.',
      kind: 'opening'
    });
  }
  return session;
...
function getOrCreateDemoSession() {
  ...
  if (!session) {
    session = createSession({
      tableName: 'Training Level 1 — Fictional Baton Rouge Venue',
...
app.get('/api/truth-packs', (_req, res) => {
  const order = ['easy', 'medium', 'hard', 'demo'];
  const packs = Object.values(truthPacks)
    .filter(p => p.gameId === 'barfly-mystery-template' && p.id !== 'barfly-mystery-template')
    .sort((a, b) => {
      const ai = order.indexOf(String(a.levelId || a.difficulty || '').toLowerCase());
      const bi = order.indexOf(String(b.levelId || b.difficulty || '').toLowerCase());
  ...
  res.json(packs.map(p => ({
    ...
    gameTitle: p.gameTitle || 'Barfly Mystery Template',
    levelId: p.levelId || '',
    ...
    titleGraphic: p.titleGraphic || p.titleImage || p.coverImage || '/assets/title-placeholder.png',
    rsvpExcerpt: p.rsvpExcerpt || p.hook || p.bookingExcerpt || p.description || ''
  })));
});
  ...
  const items = Array.from(sessions.values())
    ...
    .map(session => {
      ...
      return {
        sessionCode: session.code,
        tableName: session.tableName,
        mystery: pack.gameTitle || 'Barfly Mystery Template',
        mysteryTitle: pack.title || pack.gameTitle || 'Barfly Mystery Template',
        venue: pack.venue || 'Mid City Fictional Baton Rouge Venue • Baton Rouge, Louisiana',
        ...
        ticketPrice: normalizeTicketPrice(session.ticketPrice || session.eventPrice || ''),
        eventPriceLabel: eventPriceLabel(session),
        titleGraphic: pack.titleGraphic || pack.titleImage || pack.coverImage || '/assets/title-placeholder.png',
...
app.post('/api/access/join', async (req, res) => {
  ...
  if (isSessionStarted(session) && !session.allowLateCheckIn && !alreadyCheckedInForJoin) {
    return res.status(409).json({ error: 'The investigation has already started. Please see the host.' });
  }
