...
app.delete('/api/sessions/completed/all', async (_req, res) => {
  const completed = Array.from(sessions.values()).filter(isCompletedSession);
  const deleted = [];
  for (const session of completed) {
    const code = String(session.code || session.sessionCode || '').toUpperCase();
    if (!code) continue;
    try {
      await database.deleteSession(code);
    } catch (err) {
      console.error('Database completed-session delete failed:', err);
      return res.status(500).json({ error: 'Could not delete all completed sessions from saved storage. Refresh and try again.' });
    }
    sessions.delete(code);
    sessions.delete(String(session.code || '').toUpperCase());
    sessions.delete(String(session.sessionCode || '').toUpperCase());
    deleted.push(code);
