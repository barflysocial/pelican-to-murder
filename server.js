...
const RED_HERRING_LEVELS = ['light', 'normal', 'heavy'];
const RED_HERRING_LABELS = { light: 'Light', normal: 'Normal', heavy: 'Heavy' };

function normalizeRedHerring(value) {
  const v = String(value || 'normal').toLowerCase().trim();
  return RED_HERRING_LEVELS.includes(v) ? v : 'normal';
}

function getRedHerringLabel(value) {
  return RED_HERRING_LABELS[normalizeRedHerring(value)] || 'Normal';
}
...
function computeBriefingElapsedSec(session) {
  if (session.status !== 'briefing' || !session.briefingStartedAt) return session.briefingElapsedAtPause || 0;
  return Math.min(BRIEFING_TOTAL_SEC, Math.max(0, Math.floor((Date.now() - session.briefingStartedAt) / 1000)));
}

function computeElapsedSec(session) {
  if (session.status === 'briefing' || session.status === 'lobby') return session.elapsedAtPause || 0;
...
function getRedHerringClues(pack, mode) {
  const level = pack?.levelId || 'rookie';
  const modeKey = normalizeRedHerring(mode);
  if (modeKey === 'light') return [];

  const sharedNormal = [
    { id: `${level}-rh-normal-1`, bucket: 'messages', unlockSec: 6 * 60, title: 'Red Herring: Old Argument', text: 'A recovered message shows Cole arguing with Lena two weeks before the event. The argument sounds serious, but the timestamp is old enough that it may not explain what happened tonight.', redHerring: true },
    { id: `${level}-rh-normal-2`, bucket: 'photos', unlockSec: 14 * 60, title: 'Red Herring: Blurred Photo', text: 'A blurred photo makes Brielle look close to the service area, but the reflection in the mirror may place her farther away than it first appears.', redHerring: true }
  ];

  const heavyExtra = [
    { id: `${level}-rh-heavy-1`, bucket: 'bank', unlockSec: 8 * 60, title: 'Red Herring: Suspicious Refund', text: 'Tessa received a same-day refund from the venue. It looks suspicious until the refund code shows it was connected to a canceled table reservation.', redHerring: true },
    { id: `${level}-rh-heavy-2`, bucket: 'social', unlockSec: 12 * 60, title: 'Red Herring: Deleted Story', text: 'Marisol deleted a social post shortly after the collapse. The deletion matters, but it may be about protecting the venue instead of proving the murder.', redHerring: true },
    { id: `${level}-rh-heavy-3`, bucket: 'files', unlockSec: 18 * 60, title: 'Red Herring: Duplicate Incident Report', text: 'A duplicate incident report appears under Cole’s name. The report is messy, but the file history suggests it was copied by the office printer account, not necessarily by Cole.', redHerring: true }
  ];

  return modeKey === 'heavy' ? sharedNormal.concat(heavyExtra) : sharedNormal;
}

function visibleEvidence(session) {
  const pack = getPack(session);
  const aggregated = aggregateTruthPack(pack);
  const redHerringClues = getRedHerringClues(pack, session.redHerringLevel);
  aggregated.appClues.push(...redHerringClues);
  const elapsedSec = computeElapsedSec(session);
...
function baseBadge(score, total) {
...
function getBadge(score, total, hintUsed = false) {
  const raw = baseBadge(score, total);
  if (hintUsed && raw === 'Senior Detective') return 'Detective';
  return raw;
}

function buildHintText(session, player, pack) {
  const elapsedSec = computeElapsedSec(session);
  const { current } = currentRoundFor(session, pack);
  const usedCount = (session.hintsUsed || []).filter(h => h.playerId === player.id).length;
  const roundName = current?.shortTitle || current?.title || 'current round';
  const bank = [
    `Hint ${usedCount + 1}: Focus on the ${roundName} evidence, then compare it to the timeline. The contradiction is stronger than the rumor.`,
    `Hint ${usedCount + 1}: Do not chase the loudest suspect yet. Look for the person whose access, motive, and timing all line up.`,
    `Hint ${usedCount + 1}: One clue is only useful when paired with another app. Check the newest app evidence against an earlier clue.`,
    `Hint ${usedCount + 1}: Red herrings may point at opportunity, but the real answer needs opportunity plus proof.`
  ];
  return bank[Math.min(usedCount, bank.length - 1)];
}
...
function defaultBriefingSegments(pack = {}) {
  return [
    {
      startSec: 0,
      endSec: 60,
      title: 'The Place: Pelican to Mars',
      text: 'Pelican to Mars was built for nights people remember: neon over the bar, music low enough for secrets, and a crowd full of regulars who know exactly which faces do not belong. Tonight was supposed to be a celebration, not a crime scene. A private launch party filled the room with sponsors, influencers, staff, old friends, and people pretending their business was clean. The lights stayed warm. The drinks kept moving. But underneath the music, something had already gone wrong. By the time detectives were called, the room was still smiling for photos while one truth sat heavy in the air: someone came to the bar with a plan.'
    },
    {
      startSec: 60,
      endSec: 120,
      title: 'The Crime: A Death in the Crowd',
      text: 'The victim collapsed during the busiest part of the night, close enough to the crowd for everyone to claim they saw something, but not close enough for any story to fully match. At first, people called it a medical emergency. Someone shouted for help. Someone moved a glass. Someone said they saw her leave the main floor. Someone else swore she never did. The problem is not that there are no witnesses. The problem is that there are too many witnesses, and their timelines bend in different directions. Your job is to separate panic from performance, accident from intention, and the truth from the version everyone wants you to believe.'
    },
    {
      startSec: 120,
      endSec: 180,
      title: 'The Victim: Lena Broussard',
      text: 'Lena Broussard was not just another guest at the launch party. She was sharp, observant, and difficult to fool. She knew who had access to the private areas, who needed the sponsor money, and who had been lying about what happened before the event. In the week before her death, Lena started asking questions that made powerful people nervous. She was checking records, comparing stories, and quietly warning someone that the truth was about to come out. To some people, Lena was a friend. To others, she was a threat. By the end of the night, that threat was silenced.'
    },
    {
      startSec: 180,
      endSec: 240,
      title: 'The Circle: Suspects and Motives',
      text: 'Everyone close to Lena had a reason to control the story. Tessa Marchand knew Lena personally and understood the parts of her life that never made it online. Owen Landry had business ties that depended on the night looking successful and the books staying quiet. Brielle Hart needed the party to look perfect, because her image was tied to the event. Cole Vinet had been seen arguing with Lena before the collapse. Marisol Vega moved through the room like someone who knew more than she admitted. Some motives are emotional. Some are financial. Some are about reputation. One of them became murder.'
    },
    {
      startSec: 240,
      endSec: 300,
      title: 'Your Assignment: Think Like Detectives',
      text: 'When the investigation begins, do not chase the loudest suspect. Chase the evidence. Open each app. Compare every timeline. Read the messages carefully. Check the phone records, maps, money trail, photos, files, browser history, social posts, and statements against one another. A single clue may look harmless until it is paired with the right contradiction. Each round will test what your team noticed, and the final accusation will ask for more than a name. You must prove who did it, how it happened, why it happened, and what evidence breaks the case open. The countdown is almost over. The room is waiting.'
    }
  ];
}
...
function clientState(session) {
  ...
  return {
    ...
    redHerringLevel: normalizeRedHerring(session.redHerringLevel),
    redHerringLabel: getRedHerringLabel(session.redHerringLevel),
    sponsorPopups: session.sponsorPopups || [],
    hintsUsed: session.hintsUsed || [],
    openingNarration: pack.openingNarration || '',
    briefingSegments: pack.briefingSegments || defaultBriefingSegments(pack),
...
function createSession({ tableName = 'Table 1', truthPackId = 'pelican-to-murder-rookie', redHerringLevel = 'normal' } = {}) {
  let code = makeCode();
  ...
  const session = {
    ...
    redHerringLevel: normalizeRedHerring(redHerringLevel),
    status: 'lobby',
    startedAt: null,
    elapsedAtPause: 0,
    briefingStartedAt: null,
    briefingElapsedAtPause: 0,
    createdAt: Date.now(),
    players: new Map(),
    hostMessages: [],
    helpRequests: [],
...
async function loadSessionsFromDatabase() {
  ...
  for (const session of savedSessions) {
    ...
    if (!Array.isArray(session.sponsorPopups)) session.sponsorPopups = [];
    if (!Array.isArray(session.hintsUsed)) session.hintsUsed = [];
    session.redHerringLevel = normalizeRedHerring(session.redHerringLevel);
    if (!Array.isArray(session.submissions)) session.submissions = [];
    if (!Array.isArray(session.results)) session.results = [];
    if (!session.briefingStartedAt) session.briefingStartedAt = null;
    if (!session.briefingElapsedAtPause) session.briefingElapsedAtPause = 0;
...
function gradeSubmission(pack, player, submission, session) {
  ...
  const hintUsed = Boolean((session?.hintsUsed || []).some(h => h.playerId === player.id));
  const rawBadge = baseBadge(score, total);
  const badge = getBadge(score, total, hintUsed);
  return {
    id: nanoid(8),
    playerId: player.id,
    ...
    rawBadge,
    hintUsed,
    badgeLimitApplied: hintUsed && rawBadge !== badge,
    badgeLimitLanguage: hintUsed ? 'Hint used: badge cannot exceed Detective.' : '',
    breakdown,
    updatedAt: Date.now(),
...
function gradeAllPlayers(session) {
  ...
  for (const player of session.players.values()) {
    ...
    results.push(gradeSubmission(pack, player, submission, session));
  }
  return results.sort((a, b) => b.score - a.score || a.playerName.localeCompare(b.playerName));
}

function addHostMessage(session, { title, text, kind = 'dialog', imageUrl = '', appKey = '' }) {
  session.hostMessages = session.hostMessages || [];
  session.hostMessages.push({ id: nanoid(8), title, text, kind, imageUrl, appKey, createdAt: Date.now(), from: 'Host' });
}


function startInvestigation(session) {
  ...
  session.briefingElapsedAtPause = BRIEFING_TOTAL_SEC;
}

function revealSession(session, { addMessage = true } = {}) {
  const pack = getPack(session);
  session.results = gradeAllPlayers(session);
  session.status = 'revealed';
  session.revealed = true;
  session.elapsedAtPause = Math.max(session.elapsedAtPause || 0, computeElapsedSec(session));
  session.startedAt = null;
  session.briefingStartedAt = null;
  if (addMessage && !(session.hostMessages || []).some(m => m.kind === 'reveal')) {
    addHostMessage(session, { title: 'Case Closed', text: pack.revealScript || 'Detectives, the case is closed.', kind: 'reveal' });
  }
}

function questionIsOpen(q, elapsedSec, phase) {
  const unlock = Number(q.unlockSec || 0);
  const lock = Number(q.lockSec || (q.stage === 'final' ? ACCUSATION_LOCK_SEC : unlock + 60));
  if (q.stage === 'final') return phase === 'accusation' && elapsedSec >= unlock && elapsedSec < lock;
  return phase === 'investigation' && elapsedSec >= unlock && elapsedSec < lock;
}


function emitDueSponsorPopups(session) {
  if (session.status !== 'started') return false;
  const popups = session.sponsorPopups || [];
  if (!popups.length) return false;
  const elapsedSec = computeElapsedSec(session);
  const pack = getPack(session);
  const { rounds } = currentRoundFor(session, pack);
  let changed = false;
  for (const popup of popups) {
    if (popup.sent || !popup.scheduleRoundId || popup.scheduleRoundId === 'now') continue;
    const round = rounds.find(r => r.id === popup.scheduleRoundId || r.shortTitle === popup.scheduleRoundId || r.title === popup.scheduleRoundId);
    if (round && elapsedSec >= Number(round.startSec || 0)) {
      popup.sent = true;
      popup.sentAt = Date.now();
      addHostMessage(session, { title: popup.sponsorName ? `${popup.title} · ${popup.sponsorName}` : popup.title, text: popup.text, kind: popup.kind, imageUrl: popup.imageUrl, appKey: popup.appKey });
      changed = true;
    }
  }
  return changed;
...
setInterval(() => {
  for (const session of sessions.values()) {
    let sponsorChanged = emitDueSponsorPopups(session);
    if (sponsorChanged) saveSession(session);
    if (session.status === 'briefing' && computeBriefingElapsedSec(session) >= BRIEFING_TOTAL_SEC) {
...
app.post('/api/sessions/:code/briefing', async (req, res) => {
  const session = getSessionOr404(req.params.code, res);
  if (!session) return;
  ...
  session.elapsedAtPause = 0;
  session.briefingStartedAt = null;
  session.briefingElapsedAtPause = 0;
...
app.post('/api/sessions/:code/start', async (req, res) => {
  ...
  // Normal Start Game must always run the briefing first.
  // Only testing controls should pass skipBriefing=true.
  if (!req.body.skipBriefing) {
    session.status = 'briefing';
    session.revealed = false;
    session.results = [];
    session.startedAt = null;
...
app.post('/api/sessions/:code/settings', async (req, res) => {
  const session = getSessionOr404(req.params.code, res);
  if (!session) return;
  if (req.body.redHerringLevel) session.redHerringLevel = normalizeRedHerring(req.body.redHerringLevel);
  await saveSession(session);
  res.json(clientState(session));
  broadcast(session.code);
});
...
app.post('/api/sessions/:code/message', async (req, res) => {
  ...
  const appKey = String(req.body.appKey || '').trim() ? normalizeBucket(req.body.appKey) : '';
  addHostMessage(session, { title, text, kind, imageUrl, appKey });
  await saveSession(session);
  res.json(clientState(session));
  broadcast(session.code);
});
...
app.post('/api/sessions/:code/hint', async (req, res) => {
  const session = getSessionOr404(req.params.code, res);
  if (!session) return;
  const phase = computePhase(session);
  if (phase === 'lobby' || phase === 'briefing' || phase === 'revealed') return res.status(409).json({ error: 'Hints are only available during the investigation.' });
  const playerId = String(req.body.playerId || '');
  const player = session.players.get(playerId);
  if (!player) return res.status(404).json({ error: 'Player not found in this session.' });
  session.hintsUsed = session.hintsUsed || [];
  const pack = getPack(session);
  const hint = {
    id: nanoid(8),
    playerId,
    playerName: player.name,
    text: buildHintText(session, player, pack),
    createdAt: Date.now(),
    phase
  };
  session.hintsUsed.push(hint);
  await saveSession(session);
  res.json({ ok: true, hint, state: clientState(session) });
  broadcast(session.code);
});

app.post('/api/sessions/:code/sponsor-popup', async (req, res) => {
  const session = getSessionOr404(req.params.code, res);
  if (!session) return;
  const title = String(req.body.title || 'Sponsor Message').trim().slice(0, 120);
  const sponsorName = String(req.body.sponsorName || '').trim().slice(0, 120);
  const text = String(req.body.text || '').trim().slice(0, 1200);
  if (!text) return res.status(400).json({ error: 'Sponsor message is required.' });
  const imageUrl = String(req.body.imageUrl || '').trim().slice(0, 500);
  const appKey = String(req.body.appKey || '').trim() ? normalizeBucket(req.body.appKey) : '';
  const kind = appKey ? 'sponsor-clue' : 'sponsor-round';
  const scheduleRoundId = String(req.body.scheduleRoundId || '').trim();
  const popup = { id: nanoid(8), sponsorName, title, text, imageUrl, appKey, kind, scheduleRoundId, sent: false, createdAt: Date.now() };
  session.sponsorPopups = session.sponsorPopups || [];
  session.sponsorPopups.push(popup);
  if (!scheduleRoundId || scheduleRoundId === 'now') {
    popup.sent = true;
    popup.sentAt = Date.now();
    addHostMessage(session, { title: sponsorName ? `${title} · ${sponsorName}` : title, text, kind, imageUrl, appKey });
  }
  await saveSession(session);
  res.json({ ok: true, popup, state: clientState(session) });
  broadcast(session.code);
});
...
app.post('/api/sessions/:code/answer', async (req, res) => {
  ...
  for (const q of (pack?.accusation?.questions || [])) {
    ...
    if (selected && optionIds.has(selected) && questionIsOpen(q, elapsedSec, phase)) answers[q.id] = selected;
  }
...
app.post('/api/sessions/:code/accuse', async (req, res) => {
  ...
  for (const q of questions) {
    ...
    else if (q.stage === 'final') missingFinal.push(q.id);
  }
  if (missingFinal.length) {
    return res.status(400).json({ error: `Please answer all ${missingFinal.length} final accusation questions before submitting.` });
  }
  const submission = upsertSubmission(session, {
    id: nanoid(8),
    playerId,
