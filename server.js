...
function normalizeAccessCode(value) {
  return String(value || '').toUpperCase().replace(/[^A-Z0-9]/g, '').replace(/^PTM/, 'PTM-');
}
...
function rsvpSummary(session) {
  ...
  return {
    ...
    guests: rsvps.reduce((sum, r) => sum + Number(r.guestCount || 1), 0)
  };
}
...
function createSession({ tableName = 'Table 1', truthPackId = 'pelican-to-murder-rookie', playerCap = 25 } = {}) {
  let code = makeCode();
...
app.post('/api/rsvps', async (req, res) => {
  ...
  const guestCount = normalizeGuestCount(req.body.guestCount || 1);
