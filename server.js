...
function createSession({ tableName = 'Table 1', truthPackId = 'pelican-to-murder-rookie', playerCap = 25, eventDate = '', eventTime = '', eventDurationMinutes = 45, eventStatus = 'open', eventType = 'paid', sharedAccessCode = '', allowLateCheckIn = false } = {}) {
  let code = makeCode();
...
app.post('/api/checkins', async (req, res) => {
  ...
  if (!accessCode) return res.status(400).json({ error: 'Check-in code is required.' });
  ...
  if (!access) return res.status(404).json({ error: 'That check-in code is not valid for this session.' });
...
app.post('/api/access/join', async (req, res) => {
  ...
  if (!session || !access) return res.status(404).json({ error: 'Invalid check-in code.' });
  ...
  let matchingRsvp = normalizeRsvps(session).find(r => r.accessCode && normalizeAccessCode(r.accessCode) === accessCode);
  ...
  if (!firstName || !lastName) return res.status(400).json({ error: 'This code is missing RSVP identity. Please see the host.' });
