...
// Advance a second early so player screens never hang on 00:01.
const BRIEFING_ADVANCE_GRACE_SEC = 1;
...
async function maybeAutoStartScheduledSession(session) {
  ...
  if (now >= investigationStart - (BRIEFING_ADVANCE_GRACE_SEC * 1000)) {
    session.status = 'briefing';
...
async function maybeAutoStartGame(session) {
  if (!session || session.status !== 'briefing') return false;
  if (computeBriefingElapsedSec(session) < (BRIEFING_TOTAL_SEC - BRIEFING_ADVANCE_GRACE_SEC)) return false;
