...
- Crime Scene is filtered to scene/evidence items only; interrogation, witness, alibi, and contact-style items stay out of Crime Scene.


## Button cleanup build
- Removed the redundant Return to App button from Case Closed/badge actions.
- Crime Scene now auto-searches on open and uses Search Again for later checks instead of requiring Search Crime Scene first.
- Recap, Message Host, Detective Notes, dashboard Accuse, and Play Again controls remain removed/absent.


## Build Note — Crime Scene Strict Filter

Crime Scene now uses a whitelist-only evidence filter. Only items explicitly tagged as scene/physical/forensic evidence appear there. Interrogation, alibi, contact, suspect statement, witness statement, checkpoint, chapter, and public system items stay out of Crime Scene and remain in their proper gameplay areas.
