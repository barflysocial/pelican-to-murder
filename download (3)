...
const LEGACY_FORENSIC_APPS = ['phone','messages','maps','bank','photos','social','contacts','notes','files','browser'];
const BARFLY_APP_URL = 'https://YOUR-APP-URL.onrender.com/player/';
...
let crimeSceneFoundIds = [];
let forensicsJobs = {};
let interrogationViewed = {};
let crimeSceneMessage = '';
const FORENSICS_DURATIONS = [75, 90, 105, 120, 60];
...
if ($('rsvpPreviewCloseBtn')) $('rsvpPreviewCloseBtn').onclick = closeRsvpPreview;
...
setInterval(() => {
  if (state && currentApp === 'forensics') render();
}, 1000);
  ...
  if (navigator.share) {
    try {
      await navigator.share({
        title: 'Barfly Mystery Template',
        text: 'RSVP or join Barfly Mystery Template: A Live Detective Mystery Experience.',
        url: shareUrl
      });
...
function getSharedMysteryGraphic() {
  return '/assets/barfly-social-title-lounge-bg.png';
}
...
function updateLevelLabels(s = state) {
  ...
  if ($('topbarSubtitle')) $('topbarSubtitle').textContent = `Barfly Social Presents · Fictional Baton Rouge Venue${diff ? ` · ${diff}` : ''}`;
}
...
function showtimeCardHtml(item) {
  ...
  const titleGraphic = item.titleGraphic || getSharedMysteryGraphic();
  const venueLogo = item.venueLogoUrl || '';
  const venueLogoHtml = venueLogo ? `<span class="venueLogoBadge"><img src="${escapeHtml(venueLogo)}" alt="${escapeHtml(item.venue || item.venueName || 'Venue')} logo" loading="lazy" onerror="this.closest('.venueLogoBadge').remove()" /></span>` : '';
  const excerpt = item.rsvpExcerpt || item.description || 'A live mystery is waiting. Search the scene, question witnesses, and uncover the truth before the final accusation.';
  return `<article class="showtimeCard enhancedShowtimeCard">
    <button type="button" class="rsvpTitleGraphicButton" data-preview-code="${escapeHtml(item.sessionCode)}" aria-label="Preview ${escapeHtml(item.mysteryTitle || item.mystery || 'mystery')}">
      <img class="rsvpTitleGraphic" src="${escapeHtml(titleGraphic)}" alt="${escapeHtml(item.mysteryTitle || item.mystery || 'Mystery title graphic')}" loading="lazy" />
      <span class="rsvpPreviewHint">Tap to continue</span>
    </button>
    <div class="showtimeCardBody">
      <div class="time">${escapeHtml(item.timeLabel || 'Time TBD')}</div>
      <div class="showtimeTitleLine">${venueLogoHtml}<h4>${escapeHtml(item.mysteryTitle || item.mystery || 'Barfly Mystery Template')}</h4></div>
      <p>${escapeHtml(item.levelLabel || item.difficultyLabel || item.difficulty || 'Skill level TBD')} · ${escapeHtml(item.venue || item.venueName || 'Fictional Baton Rouge Venue')}</p>
      <p class="rsvpExcerpt">${escapeHtml(item.customSubtitle || item.subtitle || excerpt)}</p>
...
function openRsvpPreview(code) {
  ...
  const title = item.mysteryTitle || item.mystery || 'Barfly Mystery Template';
  const titleGraphic = item.titleGraphic || getSharedMysteryGraphic();
  const excerpt = item.rsvpExcerpt || item.description || 'A live mystery is waiting. Search the scene, question witnesses, and uncover the truth before the final accusation.';
  const eventType = item.eventType === 'free' ? 'Free Event' : (item.ticketPrice ? `Paid Event · $${item.ticketPrice}` : 'Paid Event');
  $('rsvpPreviewImage').src = titleGraphic;
  $('rsvpPreviewImage').alt = `${title} title graphic`;
  ...
  if (venueLogoWrap && venueLogoImg) {
    if (venueLogoUrl) {
      ...
      venueLogoImg.alt = `${item.venue || item.venueName || 'Venue'} logo`;
  ...
  $('rsvpPreviewVenue').textContent = item.venue || item.venueName || 'Fictional Baton Rouge Venue';
  $('rsvpPreviewExcerpt').textContent = item.customSubtitle || item.subtitle || excerpt;
  $('rsvpPreviewMeta').textContent = `${item.dateLabel || 'Date TBD'} · ${item.timeLabel || 'Time TBD'}`;
  $('rsvpPreviewPills').innerHTML = `<span class="pill">${escapeHtml(item.levelLabel || item.difficultyLabel || item.difficulty || 'Difficulty TBD')}</span><span class="pill">${escapeHtml(eventType)}</span><span class="pill">${escapeHtml(String(item.eventDurationMinutes || 45))} min</span>`;
...
function selectRsvpSession(code) {
  ...
  const selectedTitleGraphic = item.titleGraphic || getSharedMysteryGraphic();
  ...
  $('selectedSessionCard').innerHTML = `<div class="selectedSessionWithArt">
      <img class="selectedSessionArt" src="${escapeHtml(selectedTitleGraphic)}" alt="Barfly Social Mystery title graphic" />
      <div>
        <div class="time">Selected Showtime</div>
        ${selectedVenueLogo}
        <h3>${escapeHtml(item.mysteryTitle || item.mystery || 'Barfly Mystery Template')}</h3>
        <p><b>${escapeHtml(item.dateLabel || 'Date TBD')} · ${escapeHtml(item.timeLabel || 'Time TBD')}</b></p>
        <p>${escapeHtml(item.levelLabel || item.difficultyLabel || item.difficulty || 'Skill Level TBD')} · ${escapeHtml(item.venue || item.venueName || 'Fictional Baton Rouge Venue • Baton Rouge, Louisiana')}</p>
  ...
  try {
    ...
    if (result) {
      ...
      result.innerHTML = `
        ...
        <p><b>Session:</b> ${escapeHtml(session.tableName || session.truthPackTitle || 'Barfly Mystery Template')}</p>
...
function roundNarrativeFeedItems(s) {
  ...
  const checkpointQuestions = (s.accusation?.questions || []).filter(q => q.stage === 'checkpoint');
  const items = [];
  (s.rounds || []).forEach((round, index) => {
    ...
    const q = checkpointQuestions[index];
    const answered = q && saved[q.id];
    const recapSec = Number(q?.unlockSec || round.endSec || (startSec + 300)) + 2;
    if (showAll || answered || elapsed >= recapSec) {
      items.push({
        id: `round-recap-feed:${round.id || index}`,
        type: 'roundRecap',
        title: `${title} Recap`,
        text: round.recapDialogue || round.countdownReview || 'This round added new facts to the case. Compare the timeline, witness statements, and forensic results before moving forward.',
        unlockSec: recapSec,
        appKey: 'casefeed',
        appLabel: 'Case Feed',
        roundTitle: title
...
function caseFeedItems(s) {
  const publicItems = (s?.publicClues || []).map(c => ({ ...c, appKey: 'casefeed', appLabel: 'Case Feed' }));
...
function clueCounts(s) {
  const counts = { casefeed: caseFeedItems(s).length || 0 };
...
const STORY_BRIEFINGS = {
  easy: [
    ['0:00–0:45','The Night Goes Quiet','Barfly Mystery Template was supposed to be a live mystery preview inside Fictional Baton Rouge Venue: music, drinks, clues, and a staged crime for guests to solve. Then Lena Broussard collapsed during the event, and the room realized this was not part of the show.','This Easy briefing gives the clearest path: focus on Lena, the drink system, and who had access to the service rail.'],
    ['0:45–1:30','The Victim: Lena Broussard','Lena was the event lead helping Barfly Social test the first Barfly Mystery Template experience. She knew the staff layout, the clue schedule, and the sponsor money behind the event.','She also knew secrets. Several people needed Lena quiet before the launch became public.'],
    ...
    ['3:30–4:15','Evidence Path','Your apps will unlock texts, camera notes, receipt details, staff access logs, photos, and statements. Every clue belongs to a path, but not every path points to the same difficulty solution.','Read the difficulty label carefully. Each difficulty has its own culprit and evidence trail.'],
  ...
  medium: [
    ...
    ['2:15–3:30','Suspects With Motives','Tessa had role pressure. Owen had sponsor-money pressure. Brielle had reputation pressure. Cole had control over the sound booth. Marisol had access to staff systems.','At Medium difficulty, the solution is built from money, access, and a deliberate cover-up.'],
  ...
  detective: [
    ['0:00–0:45','Effects, Timing, and Control','At Medium difficulty, Barfly Mystery Template leans into technical control: sound cues, fog effects, booth automation, and a missing rescue item.','The killer used the event environment as a mechanism, not just a backdrop.'],
    ...
    ['2:15–3:30','Suspect Pressure','Cole Vinet had access to sound and timing. Owen and Tessa still carry obvious heat. Brielle has digital motive. Marisol understands credentials.','This difficulty asks who could create a controlled window and make a medical emergency look like chaos.'],
    ...
    ['4:15–5:00','Your Mission','Do not solve the case like a simple drink swap. Treat the venue itself as part of the weapon.','The killer did not only act in the room. The killer controlled the room.']
  ],
  senior: [
    ...
    ['4:15–5:00','Your Mission','Attack every easy conclusion. If a clue points too directly, ask who had the power to place it there.','Hard cases are solved by finding the person who controlled the evidence after the crime.']
  ],
  master: []
};
...
function render() {
  ...
  if ($('lobbyGameTitle')) $('lobbyGameTitle').textContent = state.gameTitle || 'Barfly Mystery Template';
  if ($('lobbyVenue')) $('lobbyVenue').textContent = state.venue || 'Fictional Baton Rouge Venue';
  if ($('lobbySeriesLine')) {
  ...
  if ($('lobbyBackstory')) {
    $('lobbyBackstory').textContent = state.lobbyBackstory || 'You have been called to Fictional Baton Rouge Venue after a private event turned into a crime scene. A guest is dead, the staff is shaken, and every suspect has a reason to lie. When the investigation begins, clues, suspect statements, and live scene updates will unlock in real time.';
  }
...
function renderCaseBoardStats() {
  ...
  const discovered = getDiscoveredCrimeSceneItems().length;
  const readyLabs = getForensicsResults().filter(item => item.ready).length;
  const submission = getMySubmission();
  const saved = submission?.answers || {};
  ...
  if ($('caseBoardEvidence')) $('caseBoardEvidence').textContent = `${discovered}/${getAvailableCrimeSceneItems().length}`;
  if ($('caseBoardStatements')) $('caseBoardStatements').textContent = String(statements.length + readyLabs);
...
function renderApps() {
  ...
  $('appGrid').innerHTML = Object.entries(APP_META).map(([key,[emoji,label]]) => {
    let count = 0;
    let mini = 'Open';
    if (key === 'casefeed') { count = caseFeedItems(state).length || 0; mini = `${count} updates`; }
    else if (key === 'crimeScene') { count = getDiscoveredCrimeSceneItems().length; mini = `${count}/${getAvailableCrimeSceneItems().length} found`; }
    else if (key === 'interrogation') { count = getVisibleInterrogationItems().length; mini = `${count} available`; }
    else if (key === 'forensics') { const labs = getForensicsResults(); count = labs.length; const ready = labs.filter(x => x.ready).length; mini = `${ready}/${count} ready`; }
    else if (key === 'timeline') { count = buildTimelineItems().length; mini = `${count} entries`; }
    else if (key === 'accuse') { count = getVisibleQuestions().length; mini = accusationMini(); }
    else if (key === 'detectiveNotes') { count = detectiveNotesWordCount(); mini = `${count} words saved`; }
...
function renderAppDetail() {
  ...
  if (currentApp === 'detectiveNotes') { renderDetectiveNotes(); return; }
  if (currentApp === 'crimeScene') { renderCrimeScene(); return; }
  ...
  const clues = currentApp === 'casefeed' ? caseFeedItems(state) : (state.apps?.[currentApp] || []);
...
function loadNarrativeInvestigationState() {
  try { crimeSceneFoundIds = JSON.parse(localStorage.getItem(narrativeStorageKey('crimeSceneFoundIds')) || '[]'); } catch (_err) { crimeSceneFoundIds = []; }
  try { forensicsJobs = JSON.parse(localStorage.getItem(narrativeStorageKey('forensicsJobs')) || '{}'); } catch (_err) { forensicsJobs = {}; }
  ...
  if (!Array.isArray(crimeSceneFoundIds)) crimeSceneFoundIds = [];
  if (!forensicsJobs || typeof forensicsJobs !== 'object') forensicsJobs = {};
  if (!interrogationViewed || typeof interrogationViewed !== 'object') interrogationViewed = {};
}

function saveNarrativeInvestigationState() {
  try { localStorage.setItem(narrativeStorageKey('crimeSceneFoundIds'), JSON.stringify(crimeSceneFoundIds)); } catch (_err) {}
  try { localStorage.setItem(narrativeStorageKey('forensicsJobs'), JSON.stringify(forensicsJobs)); } catch (_err) {}
...
function narrativeEvidenceSourceLabel(appKey) {
  const labels = {
    phone: 'Phone Records', messages: 'Message Extraction', maps: 'Location Data', bank: 'Bank Records',
    photos: 'Photo Evidence', social: 'Social Media', contacts: 'Contact Records', notes: 'Recovered Notes',
    files: 'Files', browser: 'Browser History', casefeed: 'Scene Report'
  };
  return labels[appKey] || 'Physical Evidence';
}

function allRawEvidenceItems(s = state) {
  const items = [];
  for (const [appKey, appClues] of Object.entries(s?.apps || {})) {
    if (!LEGACY_FORENSIC_APPS.includes(appKey)) continue;
    for (const c of (appClues || [])) {
      const type = String(c.type || '').toLowerCase();
      if (type === 'statement' || /suspect statement|alibi/i.test(`${c.title || ''} ${c.text || ''}`)) continue;
      items.push({ ...c, appKey, appLabel: narrativeEvidenceSourceLabel(appKey) });
    }
  }
  for (const c of (s?.publicClues || [])) {
    const type = String(c.type || '').toLowerCase();
    if (type === 'statement' || type === 'livescene' || /live scene|suspect statement|alibi/i.test(`${c.title || ''} ${c.text || ''}`)) continue;
    items.push({ ...c, appKey: 'casefeed', appLabel: 'Scene Report' });
  }
  return items.sort((a,b) => Number(a.unlockSec || 0) - Number(b.unlockSec || 0));
}

function getAvailableCrimeSceneItems() {
  const elapsed = Number(state?.elapsedSec || 0);
  const phase = state?.phase || 'lobby';
  if (!state || phase === 'lobby' || phase === 'briefing') return [];
  return allRawEvidenceItems(state).filter(item => phase === 'revealed' || elapsed >= Number(item.unlockSec || 0));
}

function getDiscoveredCrimeSceneItems() {
  loadNarrativeInvestigationState();
  const found = new Set(crimeSceneFoundIds);
  return getAvailableCrimeSceneItems().filter(item => found.has(item.id));
}

function getUndiscoveredCrimeSceneItems() {
  loadNarrativeInvestigationState();
  const found = new Set(crimeSceneFoundIds);
  return getAvailableCrimeSceneItems().filter(item => item.id && !found.has(item.id));
}

window.searchCrimeScene = function searchCrimeScene() {
  loadNarrativeInvestigationState();
  const next = getUndiscoveredCrimeSceneItems()[0];
  if (!next) {
    crimeSceneMessage = 'No new evidence found right now. Check back after new testimony, lab results, or a new case update.';
  } else {
    crimeSceneFoundIds.push(next.id);
    crimeSceneFoundIds = Array.from(new Set(crimeSceneFoundIds));
    crimeSceneMessage = `New evidence found: ${next.title || 'Evidence'}. Inspect it, then decide whether to send it to Forensics.`;
  }
  saveNarrativeInvestigationState();
  render();
};

function forensicDurationFor(item) {
  const idx = Math.abs(String(item?.id || '').split('').reduce((sum, ch) => sum + ch.charCodeAt(0), 0)) % FORENSICS_DURATIONS.length;
  return FORENSICS_DURATIONS[idx];
}

window.sendEvidenceToForensics = function sendEvidenceToForensics(id) {
  loadNarrativeInvestigationState();
  const item = getDiscoveredCrimeSceneItems().find(x => x.id === id) || getAvailableCrimeSceneItems().find(x => x.id === id);
  if (!item || !id) return;
  if (!forensicsJobs[id]) {
    const duration = forensicDurationFor(item);
    forensicsJobs[id] = { id, sentAt: Date.now(), readyAt: Date.now() + duration * 1000, durationSec: duration };
    crimeSceneMessage = `${item.title || 'Evidence'} sent to Forensics. You can keep investigating while the analysis runs.`;
  }
  saveNarrativeInvestigationState();
  currentApp = 'forensics';
  render();
};

function forensicStatusFor(id) {
  loadNarrativeInvestigationState();
  const job = forensicsJobs[id];
  if (!job) return null;
  const remainingMs = Math.max(0, Number(job.readyAt || 0) - Date.now());
  return { ...job, ready: remainingMs <= 0, remainingSec: Math.ceil(remainingMs / 1000) };
}
...
function forensicAnalysisText(item) {
  ...
  const natural = `${item.text || ''}${naturalEvidenceDetails(item)}`.trim();
  const title = item.title || 'the submitted evidence';
  if (item.appKey === 'messages') return `The message extraction is final. It places ${title} into the case record${storyTime ? ` around ${storyTime}` : ''}. The wording and timing can now be compared against suspect alibis.`;
  ...
  if (item.appKey === 'photos') return `The image review is final. The photo preserves what was visible at the scene${storyTime ? ` at ${storyTime}` : ''}; compare it against witness statements and the developing timeline.`;
...
function getForensicsResults() {
  loadNarrativeInvestigationState();
  const all = getAvailableCrimeSceneItems();
  return Object.keys(forensicsJobs).map(id => {
    const item = all.find(x => x.id === id) || allRawEvidenceItems(state).find(x => x.id === id) || { id, title: 'Submitted Evidence', text: '' };
    const status = forensicStatusFor(id) || { ready: false, remainingSec: 0 };
    return { ...item, ...status, analysisText: forensicAnalysisText(item) };
  }).sort((a,b) => Number(a.sentAt || 0) - Number(b.sentAt || 0));
}

function evidenceCardHtml(item) {
  const sent = Boolean(forensicsJobs[item.id]);
  const status = sent ? forensicStatusFor(item.id) : null;
  const storyTime = item.evidenceTime || item.claimTime || '';
  const naturalText = `${item.text || ''}${naturalEvidenceDetails(item)}`.trim();
  return `<div class="feedItem crimeEvidenceCard">
    <div class="time">${escapeHtml(item.appLabel || 'Physical Evidence')}${storyTime ? ' · Story Time: ' + escapeHtml(storyTime) : ''}</div>
    <h4>${escapeHtml(item.title || 'Evidence')}</h4>
    <p>${escapeHtml(naturalText || 'Evidence recovered at the scene.')}</p>
    <div class="actions">
      ${sent ? `<span class="pill miniPill">${status?.ready ? 'Forensics ready' : 'Forensics processing · ' + fmt(status?.remainingSec || 0)}</span>` : `<button class="secondary compact" type="button" onclick="sendEvidenceToForensics('${escapeHtml(item.id)}')">Send to Forensics</button>`}
    </div>
  </div>`;
}

function renderCrimeScene() {
  const [emoji,label] = APP_META.crimeScene;
  $('appTitle').textContent = `${emoji} ${label}`;
  loadNarrativeInvestigationState();
  const found = getDiscoveredCrimeSceneItems();
  const available = getAvailableCrimeSceneItems();
  const unfound = getUndiscoveredCrimeSceneItems();
  $('appEvidence').innerHTML = `<div class="narrativePanel">
    <div class="time">Discovery</div>
    <h3>Search the crime scene as the case develops.</h3>
    <p>Evidence is not all found at once. Return here after live updates, witness statements, and round progress to search again.</p>
    <div class="actions"><button type="button" onclick="searchCrimeScene()">Search Crime Scene</button></div>
    ${crimeSceneMessage ? `<p class="notice">${escapeHtml(crimeSceneMessage)}</p>` : ''}
    <p class="mini">Found ${found.length} of ${available.length} currently discoverable item${available.length === 1 ? '' : 's'}. ${unfound.length ? 'More can still be uncovered.' : 'No additional visible evidence is currently waiting.'}</p>
  </div>` + (found.length ? found.map(evidenceCardHtml).join('') : '<p class="muted">No crime scene evidence has been discovered yet. Search the scene to begin.</p>');
}

function getVisibleInterrogationItems() {
  const items = [];
  for (const c of allVisibleClues(state)) {
    const type = String(c.type || '').toLowerCase();
    if (type === 'statement' || /suspect statement|alibi|witness/i.test(`${c.title || ''} ${c.text || ''}`)) items.push(c);
  }
  return items.sort((a,b) => Number(a.unlockSec || 0) - Number(b.unlockSec || 0));
}
...
function interrogationAnswer(item, promptKey) {
  const text = item.text || 'They give a guarded statement that should be compared against the timeline.';
  if (promptKey === 'alibi') return text;
  if (promptKey === 'timeline') return `They focus on the timing of their story. Compare this statement with the Timeline and any Forensics result connected to the same window: ${text}`;
  return `Their answer adds motive, pressure, or relationship context, but it is not a confession. Use it to decide what needs to be challenged: ${text}`;
}
...
function renderInterrogation() {
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel"><div class="time">Witnesses & Suspects</div><h3>Question people. Read carefully. Challenge the story with evidence.</h3><p>Statements help fill the timeline, but they are not automatically true.</p></div>` + (items.length ? items.map(item => {
    const name = interrogationName(item);
    const prompts = [
      ['alibi','Ask about their alibi'],
      ['timeline','Ask about the timing'],
      ['motive','Ask what they noticed or what they were hiding']
    ];
    return `<div class="feedItem interrogationCard"><div class="time">Interrogation${item.claimTime ? ' · Claimed Time: ' + escapeHtml(item.claimTime) : ''}</div><h4>${escapeHtml(name || 'Witness')}</h4><p class="suspectRole">Choose a prompt, then compare the answer to Crime Scene, Forensics, and Timeline.</p><div class="promptList">${prompts.map(([key,label]) => {
...
function renderForensics() {
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel"><div class="time">Analysis</div><h3>Lab and digital work takes time.</h3><p>Send evidence from the Crime Scene. Results are final once ready and can be used to test suspect alibis.</p></div>` + (labs.length ? labs.map(item => {
    if (!item.ready) return `<div class="feedItem labProcessing"><div class="time">Forensics Processing</div><h4>${escapeHtml(item.title || 'Submitted Evidence')}</h4><p>Result ready in: <b>${fmt(item.remainingSec || 0)}</b></p><p class="mini">Keep investigating while this analysis runs.</p></div>`;
    return `<div class="feedItem labReady"><div class="time">Lab Result Ready${item.evidenceTime ? ' · Story Time: ' + escapeHtml(item.evidenceTime) : ''}</div><h4>${escapeHtml(item.title || 'Forensic Result')}</h4><p>${escapeHtml(item.analysisText)}</p></div>`;
  }).join('') : '<p class="muted">No evidence has been sent to Forensics yet. Search the Crime Scene and submit an item for analysis.</p>');
}
...
function buildTimelineItems() {
  loadNarrativeInvestigationState();
  const items = [{ id: 'body-found', time: '8:52 PM', title: 'Body found', text: 'Lena Broussard is found after the event turns from staged mystery into a real crime scene.' }];
  for (const item of getDiscoveredCrimeSceneItems()) {
    const time = item.evidenceTime || item.claimTime;
    if (time) items.push({ id: `cs-${item.id}`, time, title: item.title || 'Evidence found', text: `Crime Scene discovery: ${item.text || 'Evidence recovered.'}` });
  }
  for (const item of getVisibleInterrogationItems()) {
    const time = item.claimTime || item.evidenceTime;
    if (time) items.push({ id: `int-${item.id}`, time, title: interrogationName(item) || 'Statement', text: item.text || 'Statement recorded.' });
  }
  for (const lab of getForensicsResults().filter(x => x.ready)) {
    const time = lab.evidenceTime || lab.claimTime;
    if (time) items.push({ id: `lab-${lab.id}`, time, title: `Forensics: ${lab.title || 'Analysis'}`, text: lab.analysisText });
  }
  const seen = new Set();
  return items.filter(item => {
    const key = `${item.time}:${item.title}:${item.text}`;
    if (seen.has(key)) return false; seen.add(key); return true;
  }).sort((a,b) => parseStoryMinute(a.time) - parseStoryMinute(b.time));
}
...
function renderTimeline() {
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel"><div class="time">Case Timeline</div><h3>The timeline starts incomplete.</h3><p>Crime scene searches, interrogations, and final forensic results fill it in. Use it to challenge alibis.</p></div><div class="timelineList">${items.map(item => `<div class="timelineItem"><b>${escapeHtml(item.time || 'Time unknown')}</b><div><h4>${escapeHtml(item.title)}</h4><p>${escapeHtml(item.text)}</p></div></div>`).join('')}</div>`;
}
...
function updateDetectiveNotesStatus() {
  ...
  const base = detectiveNotesSaveStatus || 'Autosave ready.';
  el.textContent = queued ? `${base} · ${queued} case update${queued === 1 ? '' : 's'} waiting` : base;
}
...
function clueHtml(c) {
  const type = String(c.type || '').toLowerCase();
  const isRoundOpening = type === 'roundopening';
  const isRoundRecap = type === 'roundrecap';
  const isLiveScene = type === 'livescene' || /live scene update/i.test(`${c.title || ''} ${c.text || ''}`);
  const isStatement = type === 'statement' || /suspect statement|alibi/i.test(`${c.title || ''} ${c.text || ''}`);
  const storyTime = c.evidenceTime || c.claimTime || '';
  const naturalText = `${c.text || ''}${naturalEvidenceDetails(c)}`.trim();
  if (isRoundOpening) {
    return `<div class="feedItem roundOpeningCard"><div class="time">Round Opening Dialogue${c.roundTitle ? ' · ' + escapeHtml(c.roundTitle) : ''}</div><h4>${escapeHtml(c.title || 'Round Opening')}</h4><p>${escapeHtml(naturalText)}</p></div>`;
  }
  if (isRoundRecap) {
    return `<div class="feedItem roundRecapCard"><div class="time">Round Recap${c.roundTitle ? ' · ' + escapeHtml(c.roundTitle) : ''}</div><h4>${escapeHtml(c.title || 'Round Recap')}</h4><p>${escapeHtml(naturalText)}</p></div>`;
  }
  if (isLiveScene) {
    return `<div class="feedItem liveSceneCard"><div class="time">Live Scene Update${c.roundTitle ? ' · ' + escapeHtml(c.roundTitle) : ''}</div><h4>${escapeHtml(c.title || 'The room reacts.')}</h4><p>${escapeHtml(naturalText)}</p></div>`;
  }
  if (isStatement) {
...
function enqueueRoundRecapForCheckpoint(questionId) {
  if (!state || !questionId) return;
  const checkpointQuestions = (state.accusation?.questions || []).filter(q => q.stage === 'checkpoint');
  const roundIndex = checkpointQuestions.findIndex(q => q.id === questionId);
  const round = (state.rounds || [])[roundIndex];
  if (!round) return;
  const recapId = `round-recap:${state.sessionCode}:${round.id || roundIndex}`;
  const ack = getAck();
  if (ack.messages.includes(recapId)) return;
  enqueueDialog({
    key: recapId,
    meta: 'Round Recap',
    title: `${round.title || `Round ${roundIndex + 1}`} Recap`,
    text: round.recapDialogue || round.countdownReview || 'This round added new facts to the case. Compare the timeline, witness statements, and forensic results before moving forward.',
    ackType: 'message',
    ackValue: recapId
  });
...
async function submitCheckpointPopup() {
  ...
  try {
    ...
    enqueueRoundRecapForCheckpoint(qid);
    render();
    renderDialog();
  } catch (err) {
    checkpointPopupIsSubmitting = false;
...
async function renderBadgeCanvas(result) {
  ...
  const bg = await loadImage('/assets/barfly-social-title-lounge-bg.png');
  ...
  ctx.fillText('Barfly Mystery Template', centerX, 1398);
  ctx.fillStyle = '#ffd7f4';
  ctx.font = '700 31px Arial';
  ctx.fillText('Fictional Baton Rouge Venue', centerX, 1454);
...
async function shareBadge() {
  ...
  try {
    if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
      await navigator.share({ title: 'Barfly Mystery Template', text: `${result.playerName} earned the ${result.badge} badge.`, files: [file] });
    } else {
      await downloadBadge();
    }
  } catch (_err) {}
