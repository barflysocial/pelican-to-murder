...
function rsvpHtml(s) {
  ...
  const rows = rsvps.map(item => {
    ...
    return `<tr>
      ...
      <td>${escapeHtml(String(item.guestCount || 1))}</td>
  ...
  return `<div class="tableScroll"><table class="codeTable"><thead><tr><th>First</th><th>Last</th><th>Instagram</th><th>Contact</th><th>Guests</th><th>Check-In</th><th>Code</th><th>Paid</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
function tableHtml(s) {
  ...
  return `
    <article class="tableCard">
      ...
      <div class="card" style="box-shadow:none"><h3>Want to Play / RSVP List</h3><p class="mini">RSVPs save to the database even when the host dashboard is closed. Mark paid and assign an access code after payment.</p>${rsvpHtml(s)}</div>
