...
function resetCreateForm() {
  ['tableName','playerCap','eventDate','eventTime','eventDurationMinutes','sharedAccessCode','venueName','venueLogoUrl'].forEach(id => { if ($(id)) $(id).value = ''; });
...
async function loadAll() {
  ...
  $('truthPackSelect').innerHTML = '<option value="">Choose game / difficulty</option>' + packs.map(p => `<option value="${p.id}">${escapeHtml(p.gameTitle || 'Barfly Mystery Template')} — ${escapeHtml(p.levelLabel || p.difficulty || p.title)}</option>`).join('');
...
function sponsorAdHtml(s) {
  ...
  const options = [
    ...
    ['after_round_2', 'After Round 2 Checkpoint'],
    ['after_round_4', 'After Round 4 Checkpoint'],
  ...
  return `<div class="adManagerBox">
    <p class="mini">Automated sponsor popups appear at natural breaks and are dismissible. They will not interrupt Detective Notes or active checkpoint answering.</p>
    <div class="row">
...
function tableHtml(s) {
  ...
  return `
    <article class="tableCard">
      ...
      <div class="card" style="box-shadow:none"><h3>Automated Sponsor Popups</h3>${sponsorAdHtml(s)}</div>
