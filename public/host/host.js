...
async function loadAll() {
  ...
  $('truthPackSelect').innerHTML = '<option value="">Choose game level</option>' + packs.map(p => `<option value="${p.id}">${escapeHtml(p.gameTitle || 'Pelican to Murder')} — ${escapeHtml(p.levelLabel || p.difficulty || p.title)}</option>`).join('');
...
async function createSession() {
  ...
  try {
    const state = await api('/api/sessions', { method: 'POST', body: { tableName, truthPackId, playerCap, eventDate, eventTime, eventDurationMinutes, eventStatus, eventType, sharedAccessCode, allowLateCheckIn } });
    upsertSession(state);
...
function sessionTimeValue(s) {
  const raw = s.eventTimestamp || (s.eventDate ? Date.parse(`${s.eventDate}T${s.eventTime || '19:00'}:00`) : 0);
...
function hostPrimaryAction(s) {
  if (s.status === 'lobby') return `<button class="good" onclick="startTable('${s.sessionCode}')">Start Game</button>`;
  ...
  return `<button class="secondary" onclick="startTable('${s.sessionCode}')">Start</button>`;
}
function syncEventTypeFields() {
  ...
  shared.placeholder = type === 'free' ? 'Example: PELICANGARDEN' : 'Used for free events only';
}
...
function rsvpHtml(s) {
  ...
  return `<div class="tableScroll"><table class="codeTable"><thead><tr><th>First</th><th>Last</th><th>Instagram</th><th>Contact</th><th>Status</th><th>Code</th><th>Paid</th><th>Host Check-In</th><th>Code Action</th><th>Delete</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
window.startTable = async code => {
  try {
    ...
    hostNotify('Game has started. Players in the waiting room will enter the case setup.');
