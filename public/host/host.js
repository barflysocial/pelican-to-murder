...
function rememberHostDetailsState() {
  document.querySelectorAll('details[data-host-details-key]').forEach(details => {
    openHostDetails[details.dataset.hostDetailsKey] = details.open;
  });
}

function restoreHostDetailsState() {
  document.querySelectorAll('details[data-host-details-key]').forEach(details => {
    const key = details.dataset.hostDetailsKey;
    if (Object.prototype.hasOwnProperty.call(openHostDetails, key)) {
      details.open = Boolean(openHostDetails[key]);
    }
    details.ontoggle = () => {
      openHostDetails[key] = details.open;
    };
  });
}

function safeRenderHostDashboard() {
  rememberHostDetailsState();
  render();
  restoreHostDetailsState();
}
...
function connectSocket(code) {
  ...
  ws.onmessage = evt => {
    ...
    if (msg.type === 'state') {
      ...
      if (!isHostEditingDashboardSponsorAds()) safeRenderHostDashboard();
    }
  };
  ws.onclose = () => {
    sockets.delete(code);
...
function render() {
  ...
  const completedCount = sessions.filter(s => sessionBucket(s) === 'completed').length;
  ...
  const completedTools = activeHostTab === 'completed'
    ? `<div class="card completedBulkTools" style="box-shadow:none">
        <div class="row">
          <div>
            <h3>Completed Games</h3>
            <p class="mini">${completedCount ? `${completedCount} completed game${completedCount === 1 ? '' : 's'} can be deleted.` : 'No completed games to delete.'}</p>
          </div>
          <div class="actions compactButtons">
            <button class="danger" onclick="deleteAllCompletedGames()" ${completedCount ? '' : 'disabled'}>Delete All Completed Games</button>
          </div>
        </div>
      </div>`
    : '';
  $('tables').innerHTML = `${completedTools}${visible.length ? visible.map(tableHtml).join('') : `<p class="muted">${empty}</p>`}`;
}
...
function rsvpHtml(s) {
  ...
  return `<div class="tableScroll"><table class="codeTable"><thead><tr><th>First</th><th>Social</th><th>Phone</th><th>Status</th><th>Code</th><th>Payment</th><th>Host Check-In</th><th>Delete</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
function tableHtml(s) {
  const playerList = s.players.length ? `<div class="detectiveChipRow">${s.players.map(p => `<span class="pill detectiveChip ${p.connected ? 'good' : ''}"><span>${escapeHtml(fullPlayerName(p))}${p.instagram ? ` · ${escapeHtml(p.instagram)}` : ''} ${p.connected ? '●' : '○'}</span><button type="button" class="miniEditBtn" onclick="editPlayerName('${s.sessionCode}','${p.id}','${escapeHtml(p.firstName || p.name || '')}','${escapeHtml(p.lastName || '')}')">Edit</button></span>`).join('')}</div>` : '<span class="muted">No players connected.</span>';
  ...
  return `
    <article class="tableCard">
      ...
      <p class="mini">Auto-start uses the scheduled server/session time${s.scheduledStartLabel ? `: <b>${escapeHtml(s.scheduledStartLabel)}</b>` : ''}. Manual controls are only backups.</p>
      <div class="actions hostMainControls" style="margin-top:12px">
      ...
      <details class="advancedHostDetails simplifiedAdvancedControls" data-host-details-key="advanced-${s.sessionCode}">
        <summary>Advanced Controls</summary>
        <div class="card" style="box-shadow:none">
          <h3>Testing</h3>
          <div class="actions compactButtons simplifiedControlRow">
            <button class="secondary" onclick="previousRound('${s.sessionCode}')">Previous Round</button>
            <button class="secondary" onclick="nextRound('${s.sessionCode}')">Next Round</button>
            <button class="secondary" onclick="jumpToRound('${s.sessionCode}', 6)">Final</button>
          </div>
          <h3 style="margin-top:14px">Time</h3>
          <div class="actions compactButtons simplifiedControlRow smallControlRow">
            <button class="secondary" onclick="shiftTime('${s.sessionCode}', -60)">-1m</button>
            <button class="secondary" onclick="shiftTime('${s.sessionCode}', 60)">+1m</button>
          </div>
          <h3 style="margin-top:14px">Emergency</h3>
          <div class="actions compactButtons simplifiedControlRow">
          ...
          <p class="mini">Use these only for testing or emergency recovery.</p>
        </div>
      </details>
...
window.deleteAllCompletedGames = async () => {
  const completedCount = sessions.filter(s => sessionBucket(s) === 'completed').length;
  if (!completedCount) return hostNotify('No completed games to delete.', 'error');
  const ok = await hostConfirm({
    title: 'Delete All Completed Games?',
    message: `Delete all ${completedCount} completed game${completedCount === 1 ? '' : 's'} permanently? Upcoming and live games will not be touched. This cannot be undone.`,
    confirmText: 'Delete Completed Games',
    danger: true
  });
  if (!ok) return hostNotify('Delete completed games canceled.', 'error');
  try {
    const result = await api('/api/sessions/completed/all', { method: 'DELETE' });
    const deleted = new Set(result.deleted || []);
    sessions = sessions.filter(s => !deleted.has(String(s.sessionCode || s.code || '').toUpperCase()));
    safeRenderHostDashboard();
    hostNotify(`Deleted ${result.count || deleted.size || 0} completed game${(result.count || deleted.size || 0) === 1 ? '' : 's'}.`);
    await loadAll();
  } catch (err) {
    await loadAll().catch(() => {});
    hostNotify(err.message || 'Could not delete completed games.', 'error');
  }
};
...
window.jumpToRound = async (code, n) => {
  ...
  try {
    await api(`/api/sessions/${code}/set-elapsed`, { method: 'POST', body: { elapsedSec: round.startSec } });
    hostNotify(`Jumped to ${round.title}.`);
  } catch (err) { hostNotify(err.message || 'Could not jump to round.', 'error'); }
};

function activeRoundNumberForHost(code) {
  const s = sessions.find(x => x.sessionCode === code);
  if (!s) return 1;
  if (s.phase === 'accusation' || s.phase === 'accusation_locked' || s.phase === 'revealed') return 6;
  const rounds = Array.isArray(s.rounds) ? s.rounds : [];
  const current = s.currentRound || {};
  let idx = rounds.findIndex(r => String(r.id) === String(current.id));
  if (idx < 0 && Number.isFinite(Number(s.elapsedSec))) {
    const elapsed = Number(s.elapsedSec || 0);
    idx = rounds.findIndex(r => elapsed >= Number(r.startSec || 0) && elapsed < Number(r.endSec || 0));
  }
...
window.previousRound = code => {
  const current = activeRoundNumberForHost(code);
  window.jumpToRound(code, Math.max(1, current - 1));
};

window.nextRound = code => {
  const current = activeRoundNumberForHost(code);
  window.jumpToRound(code, Math.min(6, current + 1));
};
...
window.editPlayerName = async (code, playerId, currentFirst = '', currentLast = '') => {
  const firstName = prompt('Player first name:', currentFirst || '');
  if (firstName === null) return;
  const lastName = prompt('Player last name:', currentLast || '');
  if (lastName === null) return;
  if (!String(firstName || '').trim()) return hostNotify('First name is required.', 'error');
  try {
    const result = await api(`/api/sessions/${code}/players/${playerId}/name`, {
      method: 'POST',
      body: { firstName, lastName }
    });
    if (result.hostState) upsertSession(result.hostState);
    else if (result.state) upsertSession(result.state);
    safeRenderHostDashboard();
    hostNotify('Player name updated.');
  } catch (err) {
    hostNotify(err.message || 'Could not update player name.', 'error');
  }
};
