...
async function loadAll() {
  ...
  $('truthPackSelect').innerHTML = '<option value="">Choose episode / difficulty</option>' + packs.map(p => `<option value="${p.id}">${escapeHtml(p.gameTitle || 'Barfly Mystery Template')} — ${escapeHtml(p.levelLabel || p.difficulty || p.title)}</option>`).join('');
