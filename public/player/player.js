...
if ($('editPlayerNameBtn')) $('editPlayerNameBtn').onclick = openEditPlayerName;
if ($('savePlayerNameBtn')) $('savePlayerNameBtn').onclick = savePlayerName;
if ($('cancelPlayerNameBtn')) $('cancelPlayerNameBtn').onclick = closeEditPlayerName;
...
document.addEventListener('click', event => {
  ...
  // Deduction popup choices need custom handling so the popup submit button can track selection.
  ...
  // Final Accusation choices must behave like normal radio buttons.
  // Do not preventDefault and do not auto-save here; final submit collects them.
  // Final Accusation choices stay local as a draft until Lock In Final Accusation.
  // Manually checking avoids mobile/poll refresh races that can visually reset radios.
  if (input.name && input.name.startsWith('accuse-')) {
    const qid = String(input.dataset.questionId || '').trim();
    const question = (state?.accusation?.questions || []).find(q => q.id === qid);
    const isFinalChoice = state?.phase === 'accusation' || input.dataset.questionStage === 'final' || question?.stage === 'final';
    if (isFinalChoice) {
      event.preventDefault();
      input.checked = true;
      setFinalAccusationDraftAnswer(qid, input.value);
...
document.addEventListener('change', event => {
  ...
  if (name.startsWith('accuse-')) {
    ...
    // During Final Accusation, do not auto-save on radio change.
    // The Lock In Final Accusation button submits all final answers together.
    const qid = String(event.target?.dataset?.questionId || '').trim();
    const question = (state?.accusation?.questions || []).find(q => q.id === qid);
    if (state?.phase === 'accusation' || event.target?.dataset?.questionStage === 'final' || question?.stage === 'final') {
      setFinalAccusationDraftAnswer(qid, event.target.value);
      syncChoiceHighlights();
      return;
    }
...
function goHomeDashboard() {
  currentApp = null;
  if ($('appEvidence')) $('appEvidence').innerHTML = '';
  if ($('appTitle')) $('appTitle').textContent = 'Investigation Section';
  if ($('dashboardSponsorStrip')) {
    $('dashboardSponsorStrip').classList.add('hidden');
    $('dashboardSponsorStrip').innerHTML = '';
  }
  render();
...
function openEditPlayerName() {
  const me = currentPlayerRecord() || {};
  if ($('editFirstNameInput')) $('editFirstNameInput').value = me.firstName || localStorage.getItem('detectiveFirstName') || '';
  if ($('editLastNameInput')) $('editLastNameInput').value = me.lastName || '';
  if ($('editNameStatus')) $('editNameStatus').textContent = '';
  show('editNameOverlay', true);
  setTimeout(() => $('editFirstNameInput')?.focus(), 60);
}

function closeEditPlayerName() {
  show('editNameOverlay', false);
}

async function savePlayerName() {
  if (!state?.sessionCode || !playerId) return;
  const firstName = String($('editFirstNameInput')?.value || '').trim();
  const lastName = String($('editLastNameInput')?.value || '').trim();
  if (!firstName) {
    if ($('editNameStatus')) $('editNameStatus').textContent = 'First name is required.';
    return;
  }
  try {
    if ($('savePlayerNameBtn')) $('savePlayerNameBtn').disabled = true;
    if ($('editNameStatus')) $('editNameStatus').textContent = 'Saving name...';
    const result = await api(`/api/sessions/${state.sessionCode}/players/${playerId}/name`, {
      method: 'POST',
    ...
    if ($('editNameStatus')) $('editNameStatus').textContent = err.message || 'Could not update name.';
  } finally {
    if ($('savePlayerNameBtn')) $('savePlayerNameBtn').disabled = false;
  }
}
...
function isRoundDeductionQuestion(question) {
  ...
  return stage === 'deduction' || stage === 'checkpoint' || stage === 'round' || stage === 'decision' || id.startsWith('cp');
}
...
function stableRandomizedOptions(question) {
  const original = Array.isArray(question?.options) ? question.options : [];
  const options = [...original];
  if (options.length <= 1) return options;

  // The truth packs can keep the correct answer as the first option internally.
  // Player display must never show that internal first option first.
  // Order still stays stable per player/session/question so refresh does not move answers.
  const protectedFirstId = String(original[0]?.id || '');
  const seed = `${state?.sessionCode || ''}:${playerId || ''}:${question?.id || ''}:barfly-never-first-option-order-v2`;
  let n = stableHash(seed) || 1;
  ...
  if (protectedFirstId && String(options[0]?.id || '') === protectedFirstId) {
    n = (Math.imul(n, 1664525) + 1013904223) >>> 0;
    const swapIndex = 1 + (n % (options.length - 1));
...
function render() {
  ...
  const showHomeDashboard = inGame && !currentApp && !isAccusationPhase;
  const showAppDetail = inGame && currentApp && currentApp !== 'deduction' && !isAccusationPhase;
  const showDeduction = inGame && (currentApp === 'deduction' || isAccusationPhase);

  show('homeCard', showHomeDashboard);
  show('appDetailCard', showAppDetail);
  show('accuseCard', showDeduction);
  show('revealCard', isRevealed);

  if (!showAppDetail && $('appEvidence')) $('appEvidence').innerHTML = '';
  if (!showHomeDashboard && $('dashboardSponsorStrip')) {
    $('dashboardSponsorStrip').classList.add('hidden');
    $('dashboardSponsorStrip').innerHTML = '';
  }
  ...
  renderAccuse();
  // Deduction questions are only shown inside the Deduction screen; no automatic popup.
  renderReveal();
}
...
function renderDashboardSponsorStrip() {
  ...
  // The sponsor ad belongs only to the Detective Dashboard.
  // It should never appear while an app section, Deduction, Briefing, Lobby, or Reveal is open.
  const home = $('homeCard');
  const homeVisible = Boolean(home && !home.classList.contains('hidden'));
  if (!homeVisible || currentApp) {
    el.classList.add('hidden');
    el.innerHTML = '';
    return;
  }
  ...
  const logo = ad.logoUrl ? `<div class="dashboardSponsorLogoArea"><img class="dashboardSponsorLogo" src="${escapeHtml(ad.logoUrl)}" alt="${escapeHtml(ad.title || 'Sponsor')} logo" onerror="this.closest('.dashboardSponsorLogoArea')?.remove()" /></div>` : '';
...
window.openApp = key => {
  const allowed = ['forensics','interrogation','timeline','deduction'];
  const next = String(key || '').toLowerCase();
  currentApp = allowed.includes(next) ? next : null;
...
function finalDraftStorageKey() {
  return `finalAccusationDraft:${state?.sessionCode || 'session'}:${playerId || 'player'}`;
}

function getFinalAccusationDraft() {
  try {
    return JSON.parse(localStorage.getItem(finalDraftStorageKey()) || '{}') || {};
  } catch (_err) {
    return {};
  }
}

function setFinalAccusationDraftAnswer(questionId, value) {
  if (!questionId || !value) return;
  const draft = getFinalAccusationDraft();
  draft[questionId] = value;
  try { localStorage.setItem(finalDraftStorageKey(), JSON.stringify(draft)); } catch (_err) {}
}

function clearFinalAccusationDraft() {
  try { localStorage.removeItem(finalDraftStorageKey()); } catch (_err) {}
}


function restoreFinalAccusationDraftSelections() {
  const draft = getFinalAccusationDraft();
  Object.entries(draft).forEach(([questionId, value]) => {
    const input = document.querySelector(`input[name="accuse-${CSS.escape(questionId)}"][value="${CSS.escape(value)}"]`);
...
function renderAccuse() {
  ...
  const submission = getMySubmission();
  const saved = { ...(submission?.answers || {}), ...getFinalAccusationDraft() };
  ...
  setTimeout(() => {
    restoreFinalAccusationDraftSelections();
    syncChoiceHighlights();
  }, 0);
}
...
async function saveQuestionAnswer(input) {
  ...
  const overlayOpen = $('deductionOverlay') && !$('deductionOverlay').classList.contains('hidden');
  try {
    if (overlayOpen) {
    ...
    if ($('accuseResult')) $('accuseResult').textContent = `Saved. ${answeredTotal}/${total} total mystery questions answered.`;

    const qid = Object.keys(answers)[0];
    const answeredQuestion = (state.accusation?.questions || []).find(q => q.id === qid);
    if (answeredQuestion && isRoundDeductionQuestion(answeredQuestion) && state.phase !== 'accusation' && state.phase !== 'accusation_locked') {
      currentApp = null;
      if ($('appEvidence')) $('appEvidence').innerHTML = '';
      if ($('appTitle')) $('appTitle').textContent = 'Investigation Section';
      notify('Deduction saved. Returning to dashboard.');
      render();
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  } catch (err) {
    if ($('accuseResult')) $('accuseResult').textContent = err.message;
    if (overlayOpen && $('deductionPopupStatus')) $('deductionPopupStatus').textContent = err.message;
  } finally {
    deductionPopupIsSubmitting = false;
    if (overlayOpen && $('deductionPopupSubmit')) $('deductionPopupSubmit').disabled = !deductionPopupSelected;
  }
}
...
async function submitAccusation() {
  try {
    const config = state.accusation || { questions: [] };
    const submission = getMySubmission();
    const answers = { ...(submission?.answers || {}), ...getFinalAccusationDraft() };
    const missing = [];
    for (const question of config.questions || []) {
      const selected = document.querySelector(`input[name="accuse-${CSS.escape(question.id)}"]:checked`);
...
function renderDeductionPopup(forceQuestion = null) {
  // Disabled by design: no checkpoint/deduction popup.
  // Deduction questions appear only when the player taps Deduction.
...
async function submitDeductionPopup() {
  ...
  const selected = checkedInput?.value || deductionPopupSelected || '';

  if (!question || !isRoundDeductionQuestion(question)) {
    if ($('deductionPopupStatus')) $('deductionPopupStatus').textContent = 'This decision moment is no longer available. Open Deduction again.';
    return;
  }
  if (!qid || !selected) {
  ...
  try {
    ...
    show('deductionOverlay', false);
    processToastQueue();
    notify('Decision moment submitted');
    render();
    renderDialog();
  } catch (err) {
    deductionPopupIsSubmitting = false;
    if ($('deductionPopupSubmit')) $('deductionPopupSubmit').disabled = !deductionPopupSelected;
    if ($('deductionPopupStatus')) $('deductionPopupStatus').textContent = err.message || 'Could not submit this decision moment.';
  }
}
...
function inspectCountdown(next) {
  // No inter-round countdown and no deduction countdown.
  // Players move continuously through the investigation and answer only inside Deduction.
...
async function renderBadgeCanvas(result) {
  if (!result) return;
  const renderKey = `shareCardNoCircleV1:${result.playerId}:${result.updatedAt}:${result.badge}:${result.score}:${state?.gameTitle || ''}:${result.playerName || ''}`;
  if (renderKey === lastBadgeKey) return;
  lastBadgeKey = renderKey;

  const canvas = $('badgeCanvas');
  const ctx = canvas.getContext('2d');
  const bg = await loadImage('/assets/barfly-social-mystery-fullscreen-bg.png').catch(() => null);
  ...
  shade.addColorStop(0, 'rgba(3,5,12,0.60)');
  shade.addColorStop(0.48, 'rgba(3,5,12,0.78)');
  shade.addColorStop(1, 'rgba(3,5,12,0.92)');
  ctx.fillStyle = shade;
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  // Clean poster frame only. No circle, no ball, no seal.
  ctx.strokeStyle = 'rgba(255,255,255,0.18)';
  ctx.lineWidth = 4;
  roundedRect(ctx, 56, 56, canvas.width - 112, canvas.height - 112, 42);
  ctx.stroke();

  // Logo stays at the top. Everything else goes underneath.
  if (logo) {
    const maxW = canvas.width - 190;
    const maxH = 260;
    const ratio = Math.min(maxW / logo.width, maxH / logo.height);
    const w = logo.width * ratio;
    const h = logo.height * ratio;
    ctx.globalAlpha = 0.98;
    ctx.drawImage(logo, centerX - w / 2, 130, w, h);
    ctx.globalAlpha = 1;
  } else {
    ctx.textAlign = 'center';
    ctx.fillStyle = '#ffffff';
    ctx.font = '900 62px Arial';
    ctx.fillText('BARFLY SOCIAL', centerX, 245);
  }
  ...
  ctx.fillText('MYSTERY NIGHT', centerX, 460);

  ctx.fillStyle = '#ffffff';
  ctx.font = '900 88px Arial';
  wrapCenteredText(ctx, result.badge || 'Detective', centerX, 640, canvas.width - 170, 96);

  ctx.fillStyle = '#ffd7f4';
  ctx.font = '900 68px Arial';
  wrapCenteredText(ctx, result.playerName || 'Detective', centerX, 840, canvas.width - 170, 76);

  ctx.fillStyle = '#ffffff';
  ctx.font = '800 44px Arial';
  wrapCenteredText(ctx, state?.gameTitle || 'Barfly Social Mystery', centerX, 1070, canvas.width - 170, 54);

  ctx.fillStyle = '#ffd166';
  ctx.font = '900 46px Arial';
  ctx.fillText('SOLVED THE CASE', centerX, 1265);

  // Simple accent line, not a badge/seal.
  const grad = ctx.createLinearGradient(220, 1360, canvas.width - 220, 1360);
  grad.addColorStop(0, 'rgba(255,122,26,0.20)');
  grad.addColorStop(0.5, 'rgba(255,57,185,0.95)');
  grad.addColorStop(1, 'rgba(37,211,255,0.20)');
  ctx.strokeStyle = grad;
  ctx.lineWidth = 5;
  ctx.beginPath();
  ctx.moveTo(220, 1360);
  ctx.lineTo(canvas.width - 220, 1360);
  ctx.stroke();

  ctx.fillStyle = 'rgba(255,255,255,0.72)';
  ctx.font = '800 28px Arial';
  ctx.fillText('BARFLY.SOCIAL', centerX, 1575);
}
...
async function shareBadge() {
  ...
  const file = new File([blob], `barfly-mystery-badge-${safeName}.png`, { type: 'image/png' });
  try {
    if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
      await navigator.share({ title: 'Barfly Social Mystery', text: `${result.playerName} earned ${result.badge} in ${state?.gameTitle || 'Barfly Social Mystery'}.`, files: [file] });
    } else {
      await downloadBadge();
    }
  } catch (_err) {}
...
async function downloadBadge() {
  ...
  link.download = `barfly-mystery-badge-${safeName}.png`;
  link.click();
}
