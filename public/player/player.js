...
function currentSponsorSlot() {
  ...
  const id = Number(state.currentRound?.id || state.currentRound?.round || 1);
  if (id >= 5) return 'round_5';
  if (id >= 4) return 'round_4';
  if (id >= 3) return 'round_3';
  if (id >= 2) return 'round_2';
  return 'round_1';
}
