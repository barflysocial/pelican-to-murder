...
function selectRsvpSession(code) {
  ...
  $('selectedSessionCard').innerHTML = `<div class="selectedSessionVenueOnly">
      ${selectedVenueLogo}
...
function enqueueClueDialogs(clues, sessionCode) {
  // Quiet evidence updates: no modal interruption. The app badges and Case Feed update naturally.
  ...
  showToast(appLabels.length === 1 ? `${labelText} updated` : 'Investigation apps updated', `New evidence unlocked in ${labelText}.`, 'evidence');
}
...
function showToast(title, text = '', type = 'info') {
  ...
  toast.className = `toastNotice toast-${type}`;
...
function enqueueRoundRecapForCheckpoint(questionId) {
  ...
  rememberAck('message', recapId);
  showToast(`${round.title || `Round ${roundIndex + 1}`} Recap`, round.recapDialogue || round.countdownReview || 'Round recap added to the Case Feed.', 'round');
}
...
function enqueueExcitementDialogues(next, ack) {
  ...
  // It is delivered as a toast while the Case Feed and app badges carry the detail.
  if (round && elapsed >= Number(round.startSec || 0) + 20) {
    ...
    if (!ack.messages.includes(id)) {
      ...
      showToast(roundNum > 1 ? `Round ${roundNum} evidence updated` : 'Investigation is live', round.breakingUpdate || 'Review the latest evidence when ready.', 'round');
    }
  }
