...
function showtimeCardHtml(item) {
  ...
  const titleGraphic = item.titleGraphic || '/assets/title-placeholder.png';
  ...
  return `<article class="showtimeCard enhancedShowtimeCard">
    <button type="button" class="rsvpTitleGraphicButton" data-preview-code="${escapeHtml(item.sessionCode)}" aria-label="Preview ${escapeHtml(item.mysteryTitle || item.mystery || 'mystery')}">
      ...
      <span class="rsvpPreviewHint">Tap to preview</span>
    </button>
...
function selectRsvpSession(code) {
  ...
  $('selectedSessionCard').innerHTML = `<div class="selectedSessionWithArt">
      <img class="selectedSessionArt" src="${escapeHtml(item.titleGraphic || '/assets/title-placeholder.png')}" alt="${escapeHtml(item.mysteryTitle || item.mystery || 'Mystery')} title graphic" />
      <div>
        ...
        <p>${escapeHtml(item.rsvpExcerpt || item.description || 'Search the scene, question witnesses, and uncover the truth before the final accusation.')}</p>
...
const STORY_BRIEFINGS = {
  training: [
    ['0:00–0:45','The Night Goes Quiet','Barfly Mystery Template was supposed to be a live mystery preview inside Fictional Baton Rouge Venue: music, drinks, clues, and a staged crime for guests to solve. Then Lena Broussard collapsed during the event, and the room realized this was not part of the show.','This Training briefing gives the clearest path: focus on Lena, the drink system, and who had access to the service rail.'],
  ...
  detective: [
    ['0:00–0:45','Effects, Timing, and Control','At Detective level, Barfly Mystery Template leans into technical control: sound cues, fog effects, booth automation, and a missing rescue item.','The killer used the event environment as a mechanism, not just a backdrop.'],
...
async function renderBadgeCanvas(result) {
  ...
  const bg = await loadImage('/assets/title-placeholder.png');
