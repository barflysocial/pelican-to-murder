...
function getSharedMysteryGraphic() {
  return '/assets/barfly-social-mystery-shared-title.png';
}
...
function showtimeCardHtml(item) {
  ...
  const titleGraphic = getSharedMysteryGraphic();
  ...
  return `<article class="showtimeCard enhancedShowtimeCard">
    ...
    <div class="showtimeCardBody">
      ...
      <h4>${escapeHtml(item.mysteryTitle || item.mystery || 'Barfly Mystery Template')}</h4>
      <p>${escapeHtml(item.levelLabel || item.difficultyLabel || item.difficulty || 'Skill level TBD')} · ${escapeHtml(item.venue || 'Fictional Baton Rouge Venue')}</p>
...
function openRsvpPreview(code) {
  ...
  const excerpt = item.rsvpExcerpt || item.description || 'A live mystery is waiting. Search the scene, question witnesses, and uncover the truth before the final accusation.';
  const eventType = item.eventType === 'free' ? 'Free Event' : (item.ticketPrice ? `Paid Event · $${item.ticketPrice}` : 'Paid Event');
  $('rsvpPreviewImage').src = getSharedMysteryGraphic();
  $('rsvpPreviewImage').alt = 'Barfly Social Mystery title graphic';
  $('rsvpPreviewTitle').textContent = title;
  $('rsvpPreviewVenue').textContent = item.venue || 'Fictional Baton Rouge Venue';
...
function selectRsvpSession(code) {
  ...
  $('selectedSessionCard').innerHTML = `<div class="selectedSessionWithArt">
      <img class="selectedSessionArt" src="${escapeHtml(getSharedMysteryGraphic())}" alt="Barfly Social Mystery title graphic" />
      <div>
        ...
        <p>${escapeHtml(item.levelLabel || item.difficultyLabel || item.difficulty || 'Skill Level TBD')} · ${escapeHtml(item.venue || 'Fictional Baton Rouge Venue • Baton Rouge, Louisiana')}</p>
...
async function renderBadgeCanvas(result) {
  ...
  const bg = await loadImage('/assets/barfly-social-mystery-shared-title.png');
