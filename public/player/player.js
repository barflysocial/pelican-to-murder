...
const LEGACY_FORENSIC_APPS = ['phone','messages','maps','bank','photos','social','notes','files','browser'];
...
function allVisibleClues(s) {
  ...
  for (const c of (s.publicClues || [])) {
    if (isInternalSystemClue(c)) continue;
    clues.push({ ...c, appKey: 'caseUpdate', appLabel: 'Case Update' });
  }
...
function isCrimeSceneEvidenceItem(c, appKey = '') {
  const type = String(c?.type || '').toLowerCase();
  const key = String(appKey || '').toLowerCase();
  const combined = `${c?.title || ''} ${c?.text || ''}`.toLowerCase();

  // Crime Scene is now a strict whitelist, not a catch-all evidence feed.
  // Only items intentionally tagged as scene/physical/forensic evidence may appear here.
  // Interrogation, alibis, contacts, public checkpoint notices, and suspect/witness statements stay out.
  if (!c || key === 'contacts' || key === 'interrogation' || key === 'caseupdate') return false;
  if (isInternalSystemClue(c)) return false;
  if (type === 'statement' || type === 'livescene' || type === 'interrogation' || type === 'checkpoint') return false;
  if (/live scene|suspect statement|witness statement|interrogation|alibi|witness|suspect says|witness says|statement:/i.test(combined)) return false;

  const tag = String(c.evidenceGroup || c.category || c.kind || '').toLowerCase();
  const explicitlyScene = c.crimeScene === true || c.sceneEvidence === true || c.physicalEvidence === true || c.forensicEvidence === true;
  const taggedScene = /crime[-_ ]?scene|scene[-_ ]?evidence|physical[-_ ]?evidence|forensic[-_ ]?evidence|scene|physical|forensic/.test(tag);
  return Boolean(explicitlyScene || taggedScene);
}
...
function allRawEvidenceItems(s = state) {
  ...
  for (const [appKey, appClues] of Object.entries(s?.apps || {})) {
    ...
    for (const c of (appClues || [])) {
      if (!isCrimeSceneEvidenceItem(c, appKey)) continue;
      items.push({ ...c, appKey, appLabel: narrativeEvidenceSourceLabel(appKey) });
    }
  }
  for (const c of (s?.publicClues || [])) {
    if (!isCrimeSceneEvidenceItem(c, 'caseUpdate')) continue;
...
window.searchCrimeScene = function searchCrimeScene() {
  ...
  if (!next) {
    crimeSceneMessage = 'No new evidence yet. Check back after the next lead.';
  } else {
    crimeSceneFoundIds.push(next.id);
    crimeSceneFoundIds = Array.from(new Set(crimeSceneFoundIds));
    crimeSceneMessage = `Crime Scene updated: ${next.title || 'Evidence'} is now available. Inspect it, then decide whether to send it to Forensics.`;
  }
...
function autoSearchCrimeSceneOnOpen() {
  loadNarrativeInvestigationState();
  const next = getUndiscoveredCrimeSceneItems()[0];
  if (!next) {
    if (!crimeSceneMessage) crimeSceneMessage = 'No new evidence yet. Check back after the next lead.';
    saveNarrativeInvestigationState();
    return;
  }
  crimeSceneFoundIds.push(next.id);
...
function renderCrimeScene() {
  ...
  $('appTitle').textContent = `${emoji} ${label}`;
  loadNarrativeInvestigationState();
  if (!crimeSceneMessage && getDiscoveredCrimeSceneItems().length === 0) autoSearchCrimeSceneOnOpen();
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel">
    ...
    <h3>Crime scene evidence updates as the case develops.</h3>
    <p>The first search runs automatically when you open Crime Scene. Return after new physical evidence or scene details surface to search again.</p>
    ${crimeSceneMessage ? `<p class="notice">${escapeHtml(crimeSceneMessage)}</p>` : ''}
    <div class="actions"><button type="button" class="secondary compact" onclick="searchCrimeScene()">Search Again</button></div>
  ...
  </div>` + (found.length ? found.map(evidenceCardHtml).join('') : '<p class="muted">No crime scene evidence is available yet. Check back after the next lead.</p>');
}

function getVisibleInterrogationItems() {
  const items = [];
  for (const c of allVisibleClues(state)) {
    if (isInterrogationLikeItem(c)) items.push(c);
  }
  return items.sort((a,b) => Number(a.unlockSec || 0) - Number(b.unlockSec || 0));
}
