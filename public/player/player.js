...
document.addEventListener('click', event => {
  ...
  if (!btn) return;
  copyRsvpCode(btn.getAttribute('data-copy-code'));
});
...
async function submitRsvp() {
  ...
  try {
    ...
    if (code) {
      msg.innerHTML = '✅ RSVP saved. Your detective spot is reserved. Copy this check-in code and use it when you arrive.';
    } else {
      msg.innerHTML = '✅ RSVP saved. Your detective spot is reserved for this showtime. After payment, the host will assign your personal check-in code.';
    }
  } catch (err) {
...
function renderRsvpCodeBox(code, eventType = 'paid') {
  ...
  box.innerHTML = `
    ...
    <p class="mini">Use this code when you tap Check In. After check-in, you will enter the waiting room until the host starts the game.</p>
  `;
}
...
async function join() {
  ...
  if (!accessCode) {
    $('joinError').textContent = 'Enter your check-in code.';
    return;
  }
  try {
...
function renderStoryBriefingContent() {
  ...
  wrap.innerHTML = beats.map((beat, index) => {
    ...
    return `<article class="storyBeat" data-start="${start}" data-end="${end}">
      <div class="beatTime">Briefing Beat</div>
...
async function loadDetectiveNotesRemote() {
  ...
  if (currentApp === 'detectiveNotes') renderDetectiveNotes();
}
...
function scheduleDetectiveNotesSave() {
  detectiveNotesSaveStatus = 'Saving notes...';
...
function updateDetectiveNotesStatus() {
  ...
  if (el) el.textContent = detectiveNotesSaveStatus || 'Notes auto-save privately.';
}
...
async function saveDetectiveNotesRemote() {
  ...
  try {
    ...
    detectiveNotesSaveStatus = `Notes saved at ${new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}.`;
  } catch (_err) {
    detectiveNotesSaveStatus = 'Notes saved on this device. Reconnect to save to the session.';
  } finally {
...
function renderDetectiveNotes() {
  const [emoji,label] = APP_META.detectiveNotes;
  $('appTitle').textContent = `${emoji} ${label}`;
  if (detectiveNotesLoadedFor !== notesStorageKey()) {
    detectiveNotes = loadDetectiveNotesLocal();
    detectiveNotesLoadedFor = notesStorageKey();
    loadDetectiveNotesRemote().catch(() => {});
  }
  $('appEvidence').innerHTML = `<div class="detectiveNotesPanel">
    <div class="notesHeaderBox">
      ...
      <div class="notesAutosave" id="detectiveNotesStatus">${escapeHtml(detectiveNotesSaveStatus || 'Notes auto-save privately.')}</div>
    </div>
    <div class="notesGrid">
    ...
    <div class="actions">
      <button class="secondary" type="button" onclick="saveDetectiveNotesRemote()">Save Notes Now</button>
    </div>
  </div>`;
  updateDetectiveNotesStatus();
}
...
function sensoryNotesHtml(c) {
  const rows = [];
  if (c.evidenceTime) rows.push(['Story Time', c.evidenceTime]);
  if (c.claimTime) rows.push(['Claimed Time', c.claimTime]);
  if (c.weather) rows.push(['Weather', c.weather]);
  if (c.sight) rows.push(['Sight', c.sight]);
  if (c.sound) rows.push(['Sound', c.sound]);
  if (c.smell) rows.push(['Smell', c.smell]);
  if (c.physics) rows.push(['Physics', c.physics]);
  if (c.timelineNote) rows.push(['Timeline Note', c.timelineNote]);
  if (!rows.length) return '';
  return `<div class="sensoryGrid">${rows.map(([label,value]) => `<div class="sensoryNote"><b>${escapeHtml(label)}</b><span>${escapeHtml(value)}</span></div>`).join('')}</div>`;
}
...
function clueHtml(c) {
  ...
  if (isStatement) {
    ...
    return `<div class="feedItem suspectCard"><div class="time">Suspect Statement${storyTime ? ' · Story Time: ' + escapeHtml(storyTime) : ''}</div><h4>${escapeHtml(suspect || rawTitle)}</h4><p class="suspectRole">Interview statement / possible motive texture</p><p>${escapeHtml(c.text || '')}</p>${sensoryNotesHtml(c)}</div>`;
  }
  return `<div class="feedItem"><div class="time">Evidence${storyTime ? ' · Story Time: ' + escapeHtml(storyTime) : ''}</div><h4>${escapeHtml(c.title || 'Evidence')}</h4><p>${escapeHtml(c.text || '')}</p>${sensoryNotesHtml(c)}</div>`;
}
...
function renderCheckpointPopup(forceQuestion = null) {
  ...
  const roundNumber = ((state.accusation?.questions || []).filter(x => x.stage === 'checkpoint').findIndex(x => x.id === q.id) + 1) || '';
  ...
  if ($('checkpointPopupStatus')) $('checkpointPopupStatus').textContent = 'Use only the evidence and suspect statements from this round.';
  ...
  if ($('checkpointPopupChoices')) $('checkpointPopupChoices').innerHTML = ((q.options || q.choices) || []).map(choice => `
    <label class="choiceOption checkpointPopupChoice" tabindex="0" role="radio" aria-checked="false">
      <input type="radio" name="checkpoint-popup-${escapeHtml(q.id)}" value="${escapeHtml(choice.id)}" />
...
// Build marker: detective-notes-001
