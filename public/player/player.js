...
function clueHtml(c) {
  ...
  if (isStatement) {
    ...
    return `<div class="feedItem suspectCard"><div class="time">Suspect Statement · Unlocked at ${fmt(c.unlockSec || 0)}</div><h4>${escapeHtml(suspect || rawTitle)}</h4><p class="suspectRole">Interview statement / possible motive texture</p><p>${escapeHtml(c.text || '')}</p></div>`;
  }
