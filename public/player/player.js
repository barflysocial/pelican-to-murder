...
async function renderBadgeCanvas(result) {
  if (!result) return;
  const renderKey = `shareBadgeCleanV1:${result.playerId}:${result.updatedAt}:${result.badge}:${result.score}:${state?.gameTitle || ''}:${result.playerName || ''}`;
  ...
  shade.addColorStop(0, 'rgba(3,5,12,0.64)');
  shade.addColorStop(0.54, 'rgba(3,5,12,0.78)');
  ...
  if (logo) {
    ...
    ctx.drawImage(logo, centerX - w / 2, 96, w, h);
    ctx.globalAlpha = 1;
  } else {
    ctx.textAlign = 'center';
    ctx.fillStyle = '#ffffff';
    ctx.font = '900 46px Arial';
    ctx.fillText('BARFLY SOCIAL', centerX, 165);
  }
  ...
  ctx.fillText('MYSTERY NIGHT', centerX, 310);

  // Main badge seal
  const sealY = 560;
  const ring = ctx.createRadialGradient(centerX, sealY, 40, centerX, sealY, 220);
  ring.addColorStop(0, 'rgba(255,209,102,0.30)');
  ring.addColorStop(0.55, 'rgba(255,57,185,0.20)');
  ring.addColorStop(1, 'rgba(37,211,255,0.12)');
  ctx.fillStyle = ring;
  ctx.beginPath();
  ctx.arc(centerX, sealY, 215, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.arc(centerX, sealY, 178, 0, Math.PI * 2);
  ctx.stroke();

  ctx.fillStyle = '#ffd166';
  ctx.font = '900 132px Arial';
  ctx.fillText('★', centerX, sealY + 48);

  ctx.fillStyle = '#ffffff';
  ctx.font = '900 76px Arial';
  wrapCenteredText(ctx, result.badge || 'Detective', centerX, 875, canvas.width - 170, 82);

  ctx.fillStyle = '#ffd7f4';
  ctx.font = '900 58px Arial';
  wrapCenteredText(ctx, result.playerName || 'Detective', centerX, 1045, canvas.width - 170, 66);

  ctx.fillStyle = '#ffffff';
  ctx.font = '800 38px Arial';
  wrapCenteredText(ctx, state?.gameTitle || 'Barfly Social Mystery', centerX, 1245, canvas.width - 170, 48);

  ctx.fillStyle = 'rgba(255,255,255,0.78)';
  ctx.font = '800 31px Arial';
  ctx.fillText('SOLVED THE CASE', centerX, 1395);

  ctx.fillStyle = 'rgba(255,255,255,0.48)';
  ctx.font = '700 24px Arial';
  ctx.fillText('Scan. Play. Solve. Share.', centerX, 1668);
}
