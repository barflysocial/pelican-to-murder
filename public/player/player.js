...
function clearCurrentPlayerRun() {
  ...
  crimeSceneMessage = '';
  if (sessionCode) {
    ['crimeSceneFoundIds','forensicsJobs','interrogationViewed'].forEach(suffix => {
...
function resetPlayerToHome() {
  ...
  ['dialogOverlay','countdownOverlay','checkpointOverlay','rsvpPreviewOverlay','shareLinkOverlay'].forEach(id => {
    const el = $(id);
    if (el) {
...
function allVisibleClues(s) {
  const clues = [];
  for (const c of (s.publicClues || [])) clues.push({ ...c, appKey: 'caseUpdate', appLabel: 'Case Update' });
...
function caseFeedItems(s) {
  const publicItems = (s?.publicClues || []).map(c => ({ ...c, appKey: 'caseUpdate', appLabel: 'Case Update' }));
...
function enqueueClueDialogs(clues, sessionCode) {
  // Quiet evidence updates: no modal interruption. The app badges and Case Updates update naturally.
  const clean = (clues || []).filter(clue => clue?.id);
  if (!clean.length) return;
  const appLabels = Array.from(new Set(clean.map(clue => clue.appLabel || APP_META[clue.appKey]?.[1] || 'Case Updates')));
...
function isAnyModalOpen() {
  return ['dialogOverlay','checkpointOverlay','countdownOverlay','termsOverlay','shareLinkOverlay','rsvpPreviewOverlay'].some(id => {
    const el = $(id);
...
function render() {
  ...
  if (isAccusationPhase) { currentApp = 'accuse'; }
  ...
  show('appDetailCard', inGame && currentApp && currentApp !== 'accuse' && !isAccusationPhase);
  show('accuseCard', inGame && (currentApp === 'accuse' || isAccusationPhase));
  ...
  if ($('lobbyBackstory')) {
    $('lobbyBackstory').textContent = state.lobbyBackstory || 'A private event has turned into a crime scene. A guest is dead, the staff is shaken, and every suspect has a reason to lie. When the investigation begins, clues, suspect statements, and live scene updates will unlock in real time.';
  }
...
function renderApps() {
  ...
  $('appGrid').innerHTML = Object.entries(APP_META).map(([key,[emoji,label]]) => {
    let count = 0;
    let mini = 'Open';
    if (key === 'crimeScene') { count = getDiscoveredCrimeSceneItems().length; mini = `${count}/${getAvailableCrimeSceneItems().length} found`; }
    ...
    else if (key === 'timeline') { count = buildTimelineItems().length; mini = `${count} case events`; }
...
function renderLatestCaseUpdates() {
  const panel = $('latestCaseUpdatesPanel');
  if (!panel || !state) return;
  const items = caseFeedItems(state).slice(-4).reverse();
  panel.innerHTML = `<div class="latestCaseHeader"><b>Latest Case Updates</b><span>${caseFeedItems(state).length} update${caseFeedItems(state).length === 1 ? '' : 's'}</span></div>` +
    (items.length ? items.map(item => `<div class="latestCaseItem"><b>${escapeHtml(item.title || 'Case Update')}</b><span>${escapeHtml(item.text || '')}</span></div>`).join('') : '<p class="mini">Case updates will appear here as the investigation develops.</p>');
}
...
function narrativeEvidenceSourceLabel(appKey) {
  const labels = {
    ...
    files: 'Files', browser: 'Browser History', caseUpdate: 'Scene Report'
  };
...
function allRawEvidenceItems(s = state) {
  ...
  for (const c of (s?.publicClues || [])) {
    ...
    items.push({ ...c, appKey: 'caseUpdate', appLabel: 'Scene Report' });
  }
  return items.sort((a,b) => Number(a.unlockSec || 0) - Number(b.unlockSec || 0));
}
...
function getVisibleQuestionsForState(s) {
  ...
  return (s.accusation.questions || []).filter(q => {
    ...
    if (q.stage === 'checkpoint') return elapsed >= unlockSec;
    return false;
  });
}
...
function enqueueRoundRecapForCheckpoint(questionId) {
  ...
  showToast(`${round.title || `Round ${roundIndex + 1}`} Recap`, round.recapDialogue || round.countdownReview || 'Round recap added to the Case Updates.', 'round');
}
