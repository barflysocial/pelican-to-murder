...
function showtimeCardHtml(item) {
  ...
  return `<article class="showtimeCard">
    <div>
      ...
      <p>${escapeHtml(item.levelLabel || item.difficultyLabel || item.difficulty || 'Skill level TBD')} · ${escapeHtml(item.venue || 'Mid City Pelican to Mars')}</p>
...
function selectRsvpSession(code) {
  ...
  $('selectedSessionCard').innerHTML = `<div class="time">Selected Showtime</div>
    ...
    <p>${escapeHtml(item.levelLabel || item.difficultyLabel || item.difficulty || 'Skill Level TBD')} · ${escapeHtml(item.venue || 'Mid City Pelican to Mars • Baton Rouge, Louisiana')}</p>
