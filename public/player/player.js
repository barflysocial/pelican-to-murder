...
function isForensicsEvidenceItem(c) {
  ...
  if (key === 'interrogation' || key === 'contacts' || key === 'caseupdate' || key === 'timeline') return false;
  if (key === 'forensics' || type === 'forensics') return true;
...
function getVisibleForensicsEvidenceItems() {
  ...
  const items = [];
  // New truth packs store reports directly under apps.forensics.
  for (const c of (state.apps?.forensics || [])) {
    const item = { ...c, appKey: 'forensics', appLabel: c.source || narrativeEvidenceSourceLabel('forensics') };
    if (isForensicsEvidenceItem(item)) items.push(item);
  }
  // Backward compatibility for any older truth pack that still has old hidden buckets.
  for (const [appKey, appClues] of Object.entries(state.apps || {})) {
    const key = String(appKey).toLowerCase();
    if (key === 'forensics' || !LEGACY_FORENSIC_APPS.includes(key)) continue;
    for (const c of (appClues || [])) {
...
function getVisibleInterrogationItems() {
  ...
  const items = [];
  for (const c of (state.apps?.interrogation || [])) {
