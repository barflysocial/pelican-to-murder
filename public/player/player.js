...
async function requestBriefingAutoAdvance() {
  ...
  if (Number(state.briefingRemainingSec || 0) > 1) return;
...
function receiveState(next, fromPoll = false) {
  ...
  if (next?.phase === 'briefing' && Number(next.briefingRemainingSec || 0) <= 1) requestBriefingAutoAdvance();
}
