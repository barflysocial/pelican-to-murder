...
function isCrimeSceneEvidenceItem(c, appKey = '') {
  ...
  const combined = `${c?.title || ''} ${c?.text || ''}`;
  if (key === 'contacts' || key === 'interrogation') return false;
  if (isInternalSystemClue(c)) return false;
  if (type === 'statement' || type === 'livescene' || type === 'interrogation') return false;
  if (/live scene|suspect statement|witness statement|interrogation|alibi|witness/i.test(combined)) return false;
  return true;
}
...
function renderCrimeScene() {
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel">
    ...
    <p>Evidence is not all found at once. Return here after witness statements and round progress to search again.</p>
