const APP_META = {
  phone: ['📞','Phone'], messages: ['💬','Messages'], maps: ['🗺️','Maps'], bank: ['🏦','Bank'], photos: ['📷','Photos'], social: ['📱','Social'], contacts: ['👥','Contacts'], notes: ['📝','Notes'], files: ['📁','Files'], browser: ['🌐','Browser'], accuse: ['⚖️','Accuse']
};
...
function detectNotifications(next, silent) {
  ...
  if (newHostMessage) notify('New host message');
  previousCounts = counts;
  previousHostMessageCount = next.hostMessages?.length || 0;
}

function clueCounts(s) {
  const counts = { public: s.publicClues?.length || 0 };
  for (const key of Object.keys(APP_META)) counts[key] = s.apps?.[key]?.length || 0;
  return counts;
}
...
function render() {
  ...
  $('timerPill').textContent = state ? fmt(state.elapsedSec) : '00:00';
...
function renderRoundCard() {
  ...
  $('roundObjective').textContent = state.currentRound.objective || 'Review the evidence and connect the clues.';
  $('roundWindowPill').textContent = `${fmt(state.currentRound.startSec)} - ${fmt(state.currentRound.endSec)}`;
  $('roundTimeline').innerHTML = (state.rounds || []).map(r => {
    const cls = r.id === state.currentRound.id ? 'roundNode active' : (state.elapsedSec >= r.endSec ? 'roundNode done' : 'roundNode');
...
function renderHostMessages() {
  $('hostMessages').innerHTML = [...(state.hostMessages || [])].reverse().map(m => `
    <div class="feedItem hostMsg"><div class="time">${new Date(m.createdAt).toLocaleTimeString()}</div><h4>${escapeHtml(m.title || 'Host')}</h4><p>${escapeHtml(m.text)}</p></div>
  `).join('');
}

function renderApps() {
  $('appGrid').innerHTML = Object.entries(APP_META).map(([key,[emoji,label]]) => {
    const count = key === 'accuse' ? ((state.phase === 'accusation' || state.phase === 'accusation_locked') ? 1 : 0) : (state.apps?.[key]?.length || 0);
...
function renderPublicFeed() {
  ...
  $('publicFeed').innerHTML = clues.length ? clues.map(clueHtml).join('') : '<p class="muted">No public clues unlocked yet.</p>';
}
...
function clueHtml(c) {
  return `<div class="feedItem"><div class="time">Unlocked ${fmt(c.unlockSec || 0)}</div><h4>${escapeHtml(c.title || 'Evidence')}</h4><p>${escapeHtml(c.text || '')}</p></div>`;
}

function accusationMini() {
  if (state.phase === 'accusation') return 'Open now';
  if (state.phase === 'accusation_locked') return 'Locked';
  return `Opens at ${fmt(state.accusationOpenSec)}`;
}
