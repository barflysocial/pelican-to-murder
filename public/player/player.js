...
function renderRsvpCodeBox(code, eventType = 'paid') {
  ...
  box.innerHTML = `
    ...
    <p class="mini">Tap Check In Now to use this code automatically, or copy it and enter it later from the Check-In screen. After check-in, you will enter the waiting room until the game starts.</p>
  `;
}
...
async function join(accessCodeOverride = '', messageEl = null) {
  ...
  if (!accessCode) {
    if (errorTarget) errorTarget.textContent = 'Enter your check-in code.';
    return;
  }
  try {
