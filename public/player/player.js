...
function dashboardBucketForClue(clue) {
  if (!clue) return 'Timeline';
  if (isInterrogationLikeItem(clue)) return 'Interrogation';
  if (LEGACY_FORENSIC_APPS.includes(String(clue.appKey || '').toLowerCase())) return 'Forensics';
  if (String(clue.type || '').toLowerCase() === 'deduction' || String(clue.type || '').toLowerCase() === 'checkpoint') return 'Deduction';
  return 'Timeline';
}
...
function enqueueClueDialogs(clues, sessionCode) {
  ...
  const appLabels = Array.from(new Set(clean.map(clue => dashboardBucketForClue(clue))));
...
function clueCounts(s) {
  const all = allVisibleClues(s || state || {});
  return {
    forensics: all.filter(c => dashboardBucketForClue(c) === 'Forensics').length,
    interrogation: all.filter(c => dashboardBucketForClue(c) === 'Interrogation').length,
    timeline: caseFeedItems(s || state || {}).length,
    deduction: (s?.accusation?.questions || []).filter(q => isRoundDeductionQuestion(q)).length
  };
}
...
function getVisibleForensicsEvidenceItems() {
  ...
  for (const [appKey, appClues] of Object.entries(state.apps || {})) {
    ...
    for (const c of (appClues || [])) {
      ...
      if (!showAll && Number(item.unlockSec || 0) > elapsed) continue;
      items.push(item);
    }
  }
  const seen = new Set();
...
function renderInterrogation() {
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel"><div class="time">Witnesses & Suspects</div><h3>Question people. Read carefully. Challenge the story with evidence.</h3><p>Statements help fill the timeline, but they are not automatically true.</p></div>` + (items.length ? items.map(item => {
    ...
    return `<div class="feedItem interrogationCard"><div class="time">Interrogation${item.claimTime ? ' · Claimed Time: ' + escapeHtml(item.claimTime) : ''}</div><h4>${escapeHtml(name || 'Witness')}</h4><p class="suspectRole">Review each topic, then compare the answer to Forensics and Timeline.</p><div class="promptList">${prompts.map(([key,label]) => {
...
function buildTimelineItems() {
  const items = [{ id: 'body-found', time: '8:52 PM', title: 'Body found', text: 'Lena Broussard is found after the event turns from staged mystery into a real case.' }];
...
function renderTimeline() {
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel"><div class="time">Case Timeline</div><h3>The timeline starts incomplete.</h3><p>Forensics and Interrogation fill it in. Use it to challenge alibis and prepare your deductions.</p></div><div class="timelineList">${items.map(item => `<div class="timelineItem"><b>${escapeHtml(item.time || 'Time unknown')}</b><div><h4>${escapeHtml(item.title)}</h4><p>${escapeHtml(item.text)}</p></div></div>`).join('')}</div>`;
}
