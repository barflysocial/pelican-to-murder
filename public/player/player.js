...
function caseFeedItems(s) {
  ...
  return [...publicItems, ...liveSceneFeedItems(s)].sort((a, b) => Number(a.unlockSec || 0) - Number(b.unlockSec || 0));
}
...
function render() {
  ...
  if ($('lobbyGameTitle')) $('lobbyGameTitle').textContent = state.gameTitle || 'Pelican to Murder';
  if ($('lobbyVenue')) $('lobbyVenue').textContent = state.venue || 'Pelican to Mars';
  if ($('lobbySeriesLine')) {
    const chapter = state.seriesArc?.chapter || state.levelLabel || '';
    const arc = state.seriesArc?.arcTitle ? `: ${state.seriesArc.arcTitle}` : '';
    $('lobbySeriesLine').textContent = `${state.seriesTitle || 'The Pelican Files'}${chapter ? ' — ' + chapter + arc : ''}`;
  }
  if ($('lobbyBackstory')) {
    $('lobbyBackstory').textContent = state.lobbyBackstory || 'You have been called to Pelican to Mars after a private event turned into a crime scene. A guest is dead, the staff is shaken, and every suspect has a reason to lie. When the investigation begins, clues, suspect statements, and live scene updates will unlock in real time.';
  }
...
function renderApps() {
  $('appGrid').innerHTML = Object.entries(APP_META).map(([key,[emoji,label]]) => {
    let count = 0;
    if (key === 'casefeed') count = caseFeedItems(state).length || 0;
