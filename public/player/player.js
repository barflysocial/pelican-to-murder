...
if ($('playerDeductionSkipToFinalBtn')) $('playerDeductionSkipToFinalBtn').onclick = skipDemoToFinal;
...
function allVisibleClues(s) {
  ...
  for (const [appKey, appClues] of Object.entries(s.apps || {})) {
    const label = APP_META[appKey]?.[1] || narrativeEvidenceSourceLabel(appKey);
    for (const c of (appClues || [])) {
      if (isInternalSystemClue(c)) continue;
      clues.push({ ...c, appKey, appLabel: label });
    }
  }
  return clues;
}
...
function dashboardBucketForClue(clue) {
  ...
  if (INTERROGATION_APP_KEYS.includes(key) || isInterrogationLikeItem(clue)) return 'Interrogation';
  if (TIMELINE_APP_KEYS.includes(key)) return 'Timeline';
  if (LEGACY_FORENSIC_APPS.includes(key) || isForensicsEvidenceItem(clue)) return 'Forensics';
...
function clueCounts(s) {
  ...
  if (s) state = s;
  const counts = {
    forensics: getForensicsResults().length,
    interrogation: getVisibleInterrogationItems().length,
    timeline: buildTimelineItems().length,
    deduction: ((s || state)?.accusation?.questions || []).filter(q => isRoundDeductionQuestion(q)).length
  };
  state = prior;
  return counts;
}
...
const STORY_BRIEFINGS = {
  easy: [
    ...
    ['3:30–4:15','Evidence Path','Forensics will return recovered phone, message, financial, document, browser, photo, and security results as the investigation progresses. Interrogation and Timeline help test those results.','Read the difficulty label carefully. Each difficulty has its own culprit and evidence trail.'],
    ['4:15–5:00','Your Mission','You are the detective team. Follow the clues as they unlock, answer each deduction question, and save your final accusation for the end.','When the timer starts, do not chase the loudest suspect. Follow the evidence that survives comparison.']
  ],
  medium: [
    ...
    ['4:15–5:00','Your Mission','Use Deduction as each round closes. Your final accusation will ask who did it, how, why, what proves it, and which statement collapses.','Do not submit based on suspicion. Submit based on the chain.']
  ],
  junior: [
...
function renderApps() {
  ...
  $('appGrid').innerHTML = Object.entries(APP_META).map(([key,[emoji,label]]) => {
    let count = 0;
    let mini = 'Open';
    if (key === 'interrogation') { count = getVisibleInterrogationItems().length; mini = `${count} available`; }
    else if (key === 'forensics') { count = getForensicsResults().length; mini = `${count} reports`; }
...
function renderAppDetail() {
  ...
  const meta = APP_META[currentApp] || ['🔎','Investigation'];
...
function isForensicsEvidenceItem(c) {
  if (!c) return false;
  const type = String(c.type || '').toLowerCase();
  const key = String(c.appKey || '').toLowerCase();
  const combined = `${c.title || ''} ${c.text || ''}`.toLowerCase();
  if (isInternalSystemClue(c)) return false;
  if (isInterrogationLikeItem(c)) return false;
  if (type === 'statement' || type === 'livescene' || type === 'interrogation' || type === 'deduction' || type === 'roundopening') return false;
  if (key === 'interrogation' || key === 'contacts' || key === 'caseupdate') return false;
  if (!LEGACY_FORENSIC_APPS.includes(key)) return false;
  if (/deduction|deduction available|chapter|round open|host message/.test(combined)) return false;
  return true;
}
...
function getVisibleForensicsEvidenceItems() {
  ...
  // The server already returns only unlocked app evidence. Do not double-filter here,
  // because demo skips and fast polling can make the client clock lag the visible evidence.
  const items = [];
  for (const [appKey, appClues] of Object.entries(state.apps || {})) {
    if (!LEGACY_FORENSIC_APPS.includes(String(appKey).toLowerCase())) continue;
    for (const c of (appClues || [])) {
      const item = { ...c, appKey, appLabel: narrativeEvidenceSourceLabel(appKey) };
      if (!isForensicsEvidenceItem(item)) continue;
      items.push(item);
    }
  }
  const seen = new Set();
...
function getVisibleInterrogationItems() {
  ...
  for (const c of allVisibleClues(state)) {
    const key = String(c.appKey || '').toLowerCase();
    if (INTERROGATION_APP_KEYS.includes(key) || isInterrogationLikeItem(c)) items.push(c);
  }
  const seen = new Set();
  return items.filter(item => {
    const key = item.id || `${item.appKey}:${item.title}:${item.text}`;
...
function renderInterrogation() {
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel"><div class="time">Witnesses & Suspects</div><h3>Question people. Read carefully. Challenge the story with evidence.</h3><p>Statements are claims, not confirmed timeline facts. Use them to find contradictions against Forensics and the verified Timeline.</p></div>` + (items.length ? items.map(item => {
...
function renderForensics() {
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel"><div class="time">Recovered Evidence Results</div><h3>Recovered Evidence Results</h3><p>Forensics returns processed results from recovered phones, messages, financial records, documents, browser history, photos, security footage, and physical evidence as the case progresses.</p></div>` + (labs.length ? labs.map(item => {
  ...
  }).join('') : '<p class="muted">No forensic results have returned yet. New reports appear as the investigation progresses.</p>');
}
...
function buildTimelineItems() {
  ...
  // Suspect and witness statements stay in Interrogation until a hard evidence
  // source confirms the timing. Deduction compares those claims against this list.
  const items = [{
    id: 'body-found',
    time: '8:52 PM',
    title: 'Body found',
    text: 'Lena Broussard is found after the event turns from staged mystery into a real case.',
    source: 'Confirmed event'
  }];
  for (const item of (state?.apps?.timeline || [])) {
    items.push({
  ...
  for (const lab of getForensicsResults()) {
    const time = lab.evidenceTime;
    if (time) {
      items.push({
        id: `lab-${lab.id}`,
        time,
        title: `Forensics: ${lab.title || 'Analysis'}`,
        text: lab.analysisText,
        source: lab.appLabel || 'Forensics'
      });
    }
  }
  const seen = new Set();
...
function renderTimeline() {
  ...
  $('appEvidence').innerHTML = `<div class="narrativePanel"><div class="time">Verified Case Timeline</div><h3>Only confirmed events go on the timeline.</h3><p>Forensics, timestamps, photos, phone/location data, financial records, and confirmed case events appear here. Suspect statements stay in Interrogation until hard evidence supports them.</p></div><div class="timelineList">${items.map(item => `<div class="timelineItem"><b>${escapeHtml(item.time || 'Time unknown')}</b><div><h4>${escapeHtml(item.title)}</h4><p>${escapeHtml(item.text)}</p>${item.source ? `<p class="muted"><b>Source:</b> ${escapeHtml(item.source)}</p>` : ''}</div></div>`).join('')}</div>`;
}
...
function renderAccuse() {
  ...
  $('submitAccuseBtn').textContent = open ? 'Lock In Final Accusation' : 'Final Accusation Opens Later';
...
function renderDemoControls() {
  ...
  const briefing = demo && state?.phase === 'briefing';
  const playable = demo && ['investigation','accusation','accusation_locked'].includes(state?.phase || '');
  if ($('briefingDemoControls')) $('briefingDemoControls').classList.toggle('hidden', !briefing);
  if ($('roundDemoControls')) $('roundDemoControls').classList.toggle('hidden', !playable);
  if ($('deductionDemoControls')) $('deductionDemoControls').classList.toggle('hidden', !playable);
  if ($('playerSkipRoundBtn')) $('playerSkipRoundBtn').textContent = state?.phase === 'accusation' ? 'Skip Final Round' : 'Skip Round';
}
