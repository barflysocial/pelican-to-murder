...
function updateLevelLabels(s = state) {
  ...
  if ($('topbarSubtitle')) $('topbarSubtitle').textContent = `Barfly Social Presents · Pelican to Mars${diff ? ` · ${diff}` : ''}${s?.redHerringLabel ? ` · Red Herrings: ${s.redHerringLabel}` : ''}`;
}
...
function clueHtml(c) {
  return `<div class="feedItem"><div class="time">Unlocked at ${fmt(c.unlockSec || 0)}</div><h4>${escapeHtml(c.title || 'Evidence')}${c.redHerring ? '<span class="redHerringChip">Red Herring</span>' : ''}</h4><p>${escapeHtml(c.text || '')}</p></div>`;
}
...
async function useHint() {
  if (!state) return;
  const ok = confirm('Use a hint? Hints can help your investigation, but using a hint means your final badge cannot exceed Detective.');
  if (!ok) return;
  try {
    const data = await api(`/api/sessions/${state.sessionCode}/hint`, { method: 'POST', body: { playerId } });
    state = data.state;
    enqueueDialog({
      key: `hint:${data.hint.id}`,
      meta: 'Hint Used',
      title: 'Detective Hint',
      text: `${data.hint.text}\n\nBadge rule: hint used, badge cannot exceed Detective.`
    });
    render();
    renderDialog();
  } catch (err) {
    enqueueDialog({ key: `hint-error:${Date.now()}`, meta: 'Hint Unavailable', title: 'Hint Unavailable', text: err.message });
    renderDialog();
  }
}
    ...
    $('resultSummary').innerHTML = `
      <div class="resultBanner">
        <div>
          ...
          <p><b>Rating:</b> ${escapeHtml(result.badge)}</p><p><b>Difficulty:</b> ${escapeHtml(state.difficultyLabel || 'ROOKIE DETECTIVE CASE')}</p><p><b>Hints Used:</b> ${result.hintUsed ? 'Yes' : 'No'}</p>${result.badgeLimitApplied ? '<p><b>Badge Limit:</b> Hint used — badge cannot exceed Detective.</p>' : ''}
        </div>
      </div>
...
function inspectDialogTriggers(next, silent = false) {
  ...
  if (!silent) {
    unseenMessages.forEach(m => {
      const isSponsor = String(m.kind || '').startsWith('sponsor');
      const appKey = m.appKey || '';
      enqueueDialog({
        key: `msg:${m.id}`,
        meta: m.kind === 'opening' ? 'Opening Briefing' : (m.kind === 'reveal' ? 'Case Closed' : (isSponsor ? 'Sponsor Message' : 'Host Dialogue')),
        title: m.title || 'Host',
        text: m.text,
        imageUrl: m.imageUrl || '',
        ackType: 'message',
        ackValue: m.id,
        viewLabel: appKey ? 'View App' : '',
        viewAction: appKey ? () => { currentApp = appKey; render(); setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 50); } : null
      });
    });
  }
...
function renderDialog() {
  ...
  if ($('dialogImage')) {
    ...
    img.classList.toggle('hidden', !current.imageUrl);
    if (current.imageUrl) img.src = current.imageUrl;
