...
async function shareHub(){
  const shareData={title:'Barfly Social Mystery',text:'Join me for a Barfly Social mystery experience.',url:location.href};
  try{
    if(navigator.share){await navigator.share(shareData);return;}
    if(navigator.clipboard){await navigator.clipboard.writeText(location.href);alert('Link copied to clipboard.');return;}
...
document.getElementById('shareBtn').onclick=shareHub;
